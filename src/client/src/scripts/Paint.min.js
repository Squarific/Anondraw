...
Paint.prototype.previewGrid = function previewGrid (leftTop, squares, sqwidth, sqheight, gutter) {
	...
	for (var k = 0; k < squares; k++) {
		...
		vf.push([localLeftTop[0], localLeftTop[1], sqwidth * this.public.zoom, sqheight * this.public.zoom]);
...
Paint.prototype.changeToolSize = function changeToolSize (size, setinput) {
	...
	if (setinput) {
		...
		this.controls.byName["tool-size"].integerOutput.textContent = (size==1.001) ? 1.001 : size;
	}
...
Paint.prototype.tools = {
	...
	line: function line (paint, event) {
		...
		if (event.type == "mouseup" || event.type == "touchend") {
			...
			paint.addUserDrawing({
				type: "line",
				x: Math.round((paint.local.leftTopX + (paint.lastLinePoint[0] / paint.local.zoom))*1000)/1000,
				y: Math.round((paint.local.leftTopY + (paint.lastLinePoint[1] / paint.local.zoom))*1000)/1000,
				x1: Math.round((paint.local.leftTopX + (scaledCoords[0] / paint.local.zoom))*1000)/1000,
				y1: Math.round((paint.local.leftTopY + (scaledCoords[1] / paint.local.zoom))*1000)/1000,
	...
	brush: function brush (paint, event, type) {
		...
		if (event.type == "mousedown" || event.type == "touchstart") {
			paint.brushing = true;
			paint.addUserPath();
			paint.addUserPathPoint([Math.round((paint.local.leftTopX + (scaledCoords[0] / paint.local.zoom))*1000)/1000,
			                        Math.round((paint.local.leftTopY + (scaledCoords[1] / paint.local.zoom))*1000)/1000]);
		}
