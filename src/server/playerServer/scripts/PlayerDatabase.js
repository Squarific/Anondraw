...
PlayerDatabase.prototype.reset = function reset (code, pass, callback) {
	...
	this.database.query("SELECT id, users.email FROM users JOIN forgotkeys ON users.email = forgotkeys.email WHERE code = ? AND active = 1 AND created < ? - INTERVAL 1 DAY; UPDATE forgotkeys SET active = 0 WHERE code = ?", [code, new Date(), code], function (err, rows) {
		if (err) {
