...
MessageDatabase.prototype.getMessageList = function getMessageList (userId, callback) {
	this.database.query("SELECT partner, last_username FROM (SELECT DISTINCT partner FROM (SELECT toId as partner FROM messages WHERE fromId = ?), (SELECT fromId as partner FROM messages WHERE toId = ?)) JOIN users ON partner = users.id", [userId, userId], function (err, rows, fields) {
		callback(err, rows);
	});
};
