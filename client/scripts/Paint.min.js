...
Paint.prototype.addCoordDom = function addCoordDom (container) {
	...
	this.coordDiv.appendChild(document.createTextNode("x: 0 - y: 0"));
};
...
Paint.prototype.keypress = function keypress (event) {
	if (event.target == document.body) {
		...
		if (event.keyCode == 91 || event.keyCode == 44 || event.keyCode == 219 || event.keyCode == 186)
			this.changeToolSize(--this.current_size, true);
		
		if (event.keyCode == 93 || event.keyCode == 46 || event.keyCode == 221 || event.keyCode == 187)
			this.changeToolSize(++this.current_size, true);
...
Paint.prototype.keydown = function keydown (event) {
	if (event.target == document.body) {
		...
		if (this.current_tool !== "grab" && event.keyCode == 32) {
			this.previous_tool = this.current_tool;
			this.changeTool("grab");
		}

		if (this.current_tool !== "picker" && event.keyCode == 18) {
			this.previous_tool = this.current_tool;
...
Paint.prototype.keyup = function keyup (event) {
	if (event.target == document.body) {
		...
		if (this.current_tool == "grab" && event.keyCode == 32 && this.previous_tool) {
			this.changeTool(this.previous_tool);
		}

		if (this.current_tool == "picker" && event.keyCode == 18 && this.previous_tool) {
			this.changeTool(this.previous_tool);
		}
	}
};
...
Paint.prototype.tools = {
	...
	brush: function brush (paint, event, type) {
		...
		if (event.type == "mousemove" || event.type == "touchmove") {
			...
			paint.lastMovePoint[0] = targetCoords[0];
			paint.lastMovePoint[1] = targetCoords[1];
	...
	text: function text (paint, event) {
		...
		if (event.type == "mousemove" || event.type == "touchmove") {
			...
			paint.effectsCanvasCtx.fillText(paint.textToolInput.value, targetCoords[0], targetCoords[1]);

			paint.lastToolText = paint.textToolInput.value;
			paint.lastMovePoint = targetCoords;
		}
