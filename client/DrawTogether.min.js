...
c[e>>>5]|=128<<24-e%32;c[(e+64>>>9<<4)+14]=b.floor(d/4294967296);c[(e+64>>>9<<4)+15]=d;a.sigBytes=4*c.length;this._process();return this._hash},clone:function(){var b=d.clone.call(this);b._hash=this._hash.clone();return b}});a.SHA256=d._createHelper(c);a.HmacSHA256=d._createHmacHelper(c)})(Math);function DrawTogether(b,a){this.container=b;this.settings=this.utils.merge(this.utils.copy(a),this.defaultSettings);this.userSettings=QuickSettings.create(0,0,"settings");this.userSettings.setDraggable(!1);this.userSettings.setCollapsible(!1);this.playerList=[];this.moveQueue=[];this.ink=0;this.previousInk=Infinity;this.lastTimeoutError=this.lastViewDeduction=this.lastZoomWarning=this.lastBrushSizeWarning=this.lastInkWarning=0;this.lastScreenMove=Date.now();this.lastScreenMoveStartPosition;this._forceFollow;
this._autoMoveScreen;this._followingPlayer;this.network=new Network(this.settings.loadbalancer);this.account=new Account(this.settings.accountServer);this.bindSocketHandlers();this.initDom();this.gui=new Gui(b);this.updateInk();"ask"==this.settings.mode?this.openModeSelector():"askjoin"==this.settings.mode?(this.changeRoom("main"),this.openModeSelector()):"join"==this.settings.mode?(0==this.settings.room.indexOf("private_")?0==this.settings.room.indexOf("private_game_")?ga("send","event","autojoin",
"privategame"):ga("send","event","autojoin","private"):this.settings.room.indexOf("game_")?ga("send","event","autojoin","game"):ga("send","event","autojoin","public"),this.changeRoom(this.settings.room,void 0,this.settings.leftTopX,this.settings.leftTopY,!0)):"private"==this.settings.mode?(this.settings.room="private_"+Math.random().toString(36).substr(2,5),this.changeRoom(this.settings.room,void 0,0,0,!0)):"member"==this.settings.mode?this.changeRoom("member-main"):"auto"==this.settings.mode&&this.changeRoom("main");
requestAnimationFrame(this.drawLoop.bind(this));this.needsClear=!1;document.addEventListener("keydown",function(b){b.target===document.body&&(27==b.keyCode&&(this.closeAccountWindow(),this.closeShareWindow(),this.closeRoomWindow(),this.closeSettingsWindow()),65==b.keyCode&&this.advancedOptions.toggleVisibility())}.bind(this));setInterval(this.displayTip.bind(this),3E5);setTimeout(this.autoMoveScreen.bind(this),0)}DrawTogether.prototype.KICKBAN_MIN_REP=50;
...
1>this.paint.public.zoom){5E3<Date.now()-this.lastZoomWarning&&(this.chat.addMessage("Drawing while zoomed out this far is only allowed if you have more than "+this.ZOOMED_OUT_MIN_REP+" reputation."),this.lastZoomWarning=Date.now());b.removePathPoint();return}this.ink-=a;this.updateInk()}this.network.socket.emit("pp",b.point,timeoutCallback(function(a){var e=Date.now();e-this.lastTimeoutError>this.TIME_BETWEEN_TIMEOUT_WARNINGS&&(this.chat.addMessage("The server took longer than "+Math.round(this.SOCKET_TIMEOUT/
1E3)+" seconds to respond. You should probably refresh your page."),this.last_error_timestamp=e);a||b.removePathPoint()},this.SOCKET_TIMEOUT,[!1]));this.lastPathPoint=b.point}else this.displayMessage("Not your turn!"),b.removePathPoint()};
