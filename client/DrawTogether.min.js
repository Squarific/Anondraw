...
DrawTogether.prototype.bindSocketHandlers=function(){var c=this;this.network.on("connect",function(){localStorage.getItem("drawtogether-name")&&c.changeName(localStorage.getItem("drawtogether-name"))});this.network.on("disconnect",function(){if(c.current_room){var a=c.current_room;delete c.current_room;c.changeRoom(a)}});this.network.on("initname",function(a){localStorage.getItem("drawtogether-name")||c.setName(a)});this.network.on("drawing",function(a){var b=c.decodeDrawing(a.drawing);c.paint.drawDrawing("public",
b);c.setPlayerPosition(a.socketid,[b.x,b.y],Date.now())});this.network.on("playernamechange",function(a){for(var b=0;b<c.playerList.length;b++)c.playerList[b].id==a.id&&(c.playerList[b].name=a.newname);c.updatePlayerList()});this.network.on("playerlist",function(a){c.playerList=a;c.updatePlayerList()});this.network.on("leave",function(a){for(var b=0;b<c.playerList.length;b++)c.playerList[b].id==a.id&&(c.playerList.splice(b,1),b--);c.updatePlayerList()});this.network.on("join",function(a){for(var b=
0;b<c.playerList.length;b++)if(c.playerList[b].id==a.id)return;c.playerList.push(a);c.updatePlayerList()});this.network.on("reputation",function(a){for(var b=0;b<c.playerList.length;b++)c.playerList[b].id==a.id&&(c.playerList[b].reputation=a.reputation);c.updatePlayerList()});this.network.on("generalmessage",function(a){c.displayMessage(a)});this.network.on("gamestatus",function(a){c.chat.addMessage("GAME",c.usernameFromSocketid(a.currentPlayer)+" is now drawing. You have "+Math.round(a.timeLeft/
1E3)+" seconds left.");c.chat.addMessage("GAME","The word contains some of the following letters: "+a.letters.join(", "));c.displayMessage("The word contains some of the following letters: "+a.letters.join(", "));for(var b=0;b<a.players.length;b++)for(var e=0;e<c.playerList.length;e++)c.playerList[e].id==a.players[b].id&&(c.playerList[e].gamescore=a.players[b].score);c.updatePlayerList()});this.network.on("gameword",function(a){c.chat.addMessage("GAME","It is your turn now! Please draw "+a);c.displayMessage("It is your turn now! Please draw '"+
a+"' you have 60 seconds!")});this.network.on("chatmessage",function(a){a=a||{};c.chat.addMessage(a.user,a.message)});this.network.on("emote",function(a){a=a||{};c.chat.addMessage(a.user+" "+a.message)})};DrawTogether.prototype.sendMessage=function(c){this.network.socket.emit("chatmessage",c)};
...
DrawTogether.prototype.changeRoom=function(c,a){c===this.current_room?this.chat.addMessage("CLIENT","You are already in room '"+c+"'"):this.changingRoom||(this.changingRoom=!0,a=a||"",this.network.loadRoom(c+a,function(b,e){"Too many users"==b?(this.changeRoom(c,(a||0)+1),this.chat.addMessage("SERVER","Room '"+c+a+"' is full! Trying "+c+((a||0)+1))):b?this.chat.addMessage("SERVER","Failed to load room '"+c+"'. Server error: "+b):(this.paint.clear(),this.setRoom(c),this.paint.drawDrawings("public",
this.decodeDrawings(e)),this.chat.addMessage("CLIENT","Ready to draw."),this.chat.addMessage("CLIENT","Invite: http://www.anondraw.com/#"+c+a),this.removeLoading(),this.changingRoom=!1)}.bind(this)),this.setLoading(c),this.chat.addMessage("CLIENT","Changing room to '"+c+a+"'"))};
...
DrawTogether.prototype.formLogin=function(){var c=this.emailInput.value,a=this.passInput.value;this.accountError(void 0);this.account.login(c,a,function(c){c?this.accountError("Couldn't login: "+c):this.createAccountWindow()}.bind(this))};DrawTogether.prototype.formRegister=function(){var c=this.emailInput.value,a=this.passInput.value;this.accountError(void 0);this.account.register(c,a,function(c){c?this.accountError("Couldn't register: "+c):this.createAccountWindow()}.bind(this))};
