...
DrawTogether.prototype.drawLoop=function(){this.userCtx.clearRect(0,0,this.userCtx.canvas.width,this.userCtx.canvas.height);for(var c=0;c<this.playerList.length;c++)this.playerList[c].lastPosition&&1500>Date.now()-this.playerList[c].lastPosition.time&&this.drawPlayerInteraction(this.playerList[c].name,this.playerList[c].lastPosition.pos);requestAnimationFrame(this.drawLoop.bind(this))};
...
DrawTogether.prototype.bindSocketHandlers=function(c){var a=this;this.network.on("connect",function(){localStorage.getItem("drawtogether-name")&&this.changeName(localStorage.getItem("drawtogether-name"))});this.network.on("initname",function(c){localStorage.getItem("drawtogether-name")||a.setName(c)});this.network.on("drawing",function(c){var e=a.decodeDrawing(c.drawing);a.paint.drawDrawing("public",e);a.setPlayerPosition(c.socketid,[e.x,e.y],Date.now())});this.network.on("playernamechange",function(c){for(var e=
0;e<a.playerList.length;e++)a.playerList[e].id==c.id&&(a.playerList[e].name=c.newname);a.updatePlayerList()});this.network.on("playerlist",function(c){a.playerList=c;a.updatePlayerList()});this.network.on("leave",function(c){for(var e=0;e<a.playerList.length;e++)a.playerList[e].id==c.id&&(a.playerList.splice(e,1),e--);a.updatePlayerList()});this.network.on("join",function(c){for(var e=0;e<a.playerList.length;e++)if(a.playerList[e].id==c.id)return;a.playerList.push(c);a.updatePlayerList()});this.network.on("reputation",
function(c){for(var e=0;e<a.playerList.length;e++)a.playerList[e].id==c.id&&(a.playerList[e].reputation=c.reputation);a.updatePlayerList()});this.network.on("generalmessage",function(c){a.displayMessage(c)});this.network.on("gamestatus",function(c){a.chat.addMessage("GAME",a.usernameFromSocketid(c.currentPlayer)+" is now drawing. You have "+Math.round(c.timeLeft/1E3)+" seconds left.");a.chat.addMessage("GAME","The word contains some of the following letters: "+c.letters.join(", "));a.displayMessage("The word contains some of the following letters: "+
c.letters.join(", "));for(var e=0;e<c.players.length;e++)for(var d=0;d<a.playerList.length;d++)a.playerList[d].id==c.players[e].id&&(a.playerList[d].gamescore=c.players[e].score);a.updatePlayerList()});this.network.on("gameword",function(c){a.chat.addMessage("GAME","It is your turn now! Please draw "+c);a.displayMessage("It is your turn now! Please draw '"+c+"' you have 60 seconds!")});this.network.on("chatmessage",function(c){c=c||{};a.chat.addMessage(c.user,c.message)});this.network.on("emote",
function(c){c=c||{};a.chat.addMessage(c.user+" "+c.message)})};DrawTogether.prototype.sendMessage=function(c){this.network.socket.emit("chatmessage",c)};DrawTogether.prototype.displayMessage=function(c,a){this.messageDom.style.display="block";this.messageDom.innerText=c;this.messageDom.textContent=c;clearTimeout(this.removeMessageTimeout);this.removeMessageTimeout=setTimeout(function(){this.messageDom.style.display=""}.bind(this),a||Math.max(1E3*Math.ceil(c.length/10),3E3))};
DrawTogether.prototype.changeRoom=function(c,a){c===this.current_room?this.chat.addMessage("CLIENT","You are already in room '"+c+"'"):(a=a||"",this.network.loadRoom(c+a,function(b,e){"Too many users"==b?this.changeRoom(c,(a||0)+1):b?this.chat.addMessage("SERVER","Failed to load room '"+c+"'. Server error: "+b):(this.paint.clear(),this.setRoom(c),self.paint.drawDrawings("public",self.decodeDrawings(e)),self.chat.addMessage("CLIENT","Ready to draw."),this.chat.addMessage("CLIENT","Invite: http://www.anondraw.com/#"+
c+a),self.removeLoading())}.bind(this)),this.setLoading(c),this.chat.addMessage("CLIENT","Changing room to '"+c+a+"'"))};DrawTogether.prototype.setLoading=function(c){this.loading||(this.loading=this.paintContainer.appendChild(document.createElement("div")),this.loading.innerText="Loading room '"+c+"' ...",this.loading.textContent="Loading room '"+c+"' ...",this.loading.className="drawtogether-loading")};
...
DrawTogether.prototype.setPlayerPosition=function(c,a,b){for(var e=0;e<this.playerList.length;e++)this.playerList[e].id==c&&(this.playerList[e].lastPosition=this.playerList[e].lastPosition||{},this.playerList[e].lastPosition.pos=a,this.playerList[e].lastPosition.time=b)};DrawTogether.prototype.sendDrawing=function(c,a){this.socket&&this.network.socket.emit("drawing",this.encodeDrawing(c),a)};
...
e.className="drawtogether-question-answer";for(var d=a[b].answer.split("\n"),f=0;f<d.length;f++){var h=e.appendChild(document.createElement("div"));h.innerText=d[f];h.textContent=d[f]}}return c};
...
Gui.prototype.prompt=function(c,a,b){"function"==typeof a&&(b=a,a="freepick");b="function"==typeof b?b:function(){};var e=this.container.appendChild(document.createElement("div")),d=e.appendChild(document.createElement("div")),f=d.appendChild(document.createElement("span")),h=d.appendChild(document.createElement("div"));e.className="gui-prompt-container";d.className="gui-prompt-content";f.className="gui-prompt-question";h.className="gui-prompt-answers";f.innerText=c;f.textContent=c;"freepick"==a&&
h.appendChild(this.createFreePick(c,function(c){b(c);this.container.removeChild(e)}.bind(this)));if("object"==typeof a&&"number"==typeof a.length)for(d=0;d<a.length;d++)"freepick"==a[d]?h.appendChild(this.createFreePick(c,function(c){b(c);this.container.removeChild(e)}.bind(this))):(f=h.appendChild(document.createElement("div")),f.className="gui-prompt-button gui-prompt-option-button",f.innerText=a[d],f.textContent=a[d],f.option=a[d],f.addEventListener("click",function(c){this.container.removeChild(e);
b(c.target.option)}.bind(this)))};Gui.prototype.createFreePick=function(c,a){var b=document.createElement("div"),e=b.appendChild(document.createElement("input"));e.className="gui-prompt-freepick-input";e.placeholder=c;e.addEventListener("keypress",function(c){13==c.keyCode&&a(e.value)});var d=b.appendChild(document.createElement("div"));d.className="gui-prompt-button gui-prompt-freepick-button";d.innerText="Submit";d.textContent="Submit";d.addEventListener("click",function(c){a(e.value)});return b};function Network(c){this.mainServer=c;this.socket;this.callbacks={}}Network.prototype.loadRoom=function(c,a){this.getServerFromRoom(c,function(b,e){b?a(b):(this.changeServer(e),this.socket.emit("changeroom",c,a))}.bind(this))};
...
b.send()};Network.prototype.changeServer=function(c){this.socket.io.uri!==c&&(this.socket.destroy(),delete this.socket);this.socket||(this.socket=io(c,{transports:["websocket"]}),this.reBindHandlers())};Network.prototype.on=function(c,a){this.callbacks[c]=this.callbacks[c]||[];-1===this.callbacks[c].indexOf(a)&&(this.callbacks[c].push(a),this.socket.on(c,a))};
...
b)));return c};
(function(c){"function"===typeof define&&define.amd?define(["jquery"],c):"object"==typeof exports&&"object"==typeof module?module.exports=c:c(jQuery)})(function(c,a){function b(a,b,d,e){for(var n=[],f=0;f<a.length;f++){var h=a[f];if(h){var g=tinycolor(h),x=0.5>g.toHsl().l?"sp-thumb-el sp-thumb-dark":"sp-thumb-el sp-thumb-light",x=x+(tinycolor.equals(b,h)?" sp-thumb-active":""),h=g.toString(e.preferredFormat||"rgb"),s=k?"background-color:"+g.toRgbString():"filter:"+g.toFilter();n.push('<span title="'+h+
'" data-color="'+g.toRgbString()+'" class="'+x+'"><span class="sp-thumb-inner" style="'+s+';" /></span>')}else n.push(c("<div />").append(c('<span data-color="" style="background-color:transparent;" class="sp-clear-display"></span>').attr("title",e.noColorSelectedText)).html())}return"<div class='sp-cf "+d+"'>"+n.join("")+"</div>"}function e(a,b){var d=c.extend({},s,a);d.callbacks={move:m(d.move,b),change:m(d.change,b),show:m(d.show,b),hide:m(d.hide,b),beforeShow:m(d.beforeShow,b)};return d}function d(d,
h){function s(){p.showPaletteOnly&&(p.showPalette=!0);Ba.text(p.showPaletteOnly?p.togglePaletteMoreText:p.togglePaletteLessText);if(p.palette){qa=p.palette.slice(0);ga=c.isArray(qa[0])?qa:[qa];ra={};for(var a=0;a<ga.length;a++)for(var b=0;b<ga[a].length;b++){var A=tinycolor(ga[a][b]).toRgbString();ra[A]=!0}}v.toggleClass("sp-flat",M);v.toggleClass("sp-input-disabled",!p.showInput);v.toggleClass("sp-alpha-enabled",p.showAlpha);v.toggleClass("sp-clear-enabled",Y);v.toggleClass("sp-buttons-disabled",
!p.showButtons);v.toggleClass("sp-palette-buttons-disabled",!p.togglePaletteOnly);v.toggleClass("sp-palette-disabled",!p.showPalette);v.toggleClass("sp-palette-only",p.showPaletteOnly);v.toggleClass("sp-initial-disabled",!p.showInitial);v.addClass(p.className).addClass(p.containerClassName);J()}function m(){if(t&&window.localStorage){try{var a=window.localStorage[t].split(",#");1<a.length&&(delete window.localStorage[t],c.each(a,function(c,a){fa(a)}))}catch(b){}try{N=window.localStorage[t].split(";")}catch(A){}}}
function fa(a){if(R){a=tinycolor(a).toRgbString();if(!ra[a]&&-1===c.inArray(a,N))for(N.push(a);N.length>Ua;)N.shift();if(t&&window.localStorage)try{window.localStorage[t]=N.join(";")}catch(b){}}}function Ka(){var c=[];if(p.showPalette)for(var a=0;a<N.length;a++){var b=tinycolor(N[a]).toRgbString();ra[b]||c.push(N[a])}return c.reverse().slice(0,p.maxSelectionSize)}function I(){var a=E(),A=c.map(ga,function(c,A){return b(c,a,"sp-palette-row sp-palette-row-"+A,p)});m();N&&A.push(b(Ka(),a,"sp-palette-row sp-palette-row-selection",
p));La.html(A.join(""))}function w(){if(p.showInitial){var c=Z,a=E();Ma.html(b([c,a],a,"sp-palette-row-initial",p))}}function L(){(0>=K||0>=O||0>=sa)&&J();v.addClass(Na);S=null;B.trigger("dragstart.spectrum",[E()])}function z(){v.removeClass(Na);B.trigger("dragstop.spectrum",[E()])}function W(){var c=T.val();null!==c&&""!==c||!Y?(c=tinycolor(c),c.isValid()?(U(c),H(!0)):T.addClass("sp-validation-error")):(U(null),H(!0))}function ta(){r?aa():oa()}function oa(){var a=c.Event("beforeShow.spectrum");if(r)J();
else if(B.trigger(a,[E()]),!1!==C.beforeShow(E())&&!a.isDefaultPrevented()){for(a=0;a<u.length;a++)u[a]&&u[a].hide();r=!0;c(Ca).bind("click.spectrum",Aa);c(window).bind("resize.spectrum",ba);ca.addClass("sp-active");v.removeClass("sp-hidden");J();Q();Z=E();w();C.show(Z);B.trigger("show.spectrum",[Z])}}function Aa(c){2!=c.button&&(Va?H(!0):U(Z,!0),aa())}function aa(){r&&!M&&(r=!1,c(Ca).unbind("click.spectrum",Aa),c(window).unbind("resize.spectrum",ba),ca.removeClass("sp-active"),v.addClass("sp-hidden"),
C.hide(E()),B.trigger("hide.spectrum",[E()]))}function U(c,a){if(tinycolor.equals(c,E()))Q();else{var b,A;!c&&Y?V=!0:(V=!1,b=tinycolor(c),A=b.toHsv(),ha=A.h%360/360,ia=A.s,ja=A.v,P=A.a);Q();b&&(b.isValid()&&!a)&&(ka=Da||b.getFormat())}}function E(c){c=c||{};return Y&&V?null:tinycolor.fromRatio({h:ha,s:ia,v:ja,a:Math.round(100*P)/100},{format:c.format||ka})}function da(){Q();C.move(E());B.trigger("move.spectrum",[E()])}function Q(){T.removeClass("sp-validation-error");y();var c=tinycolor.fromRatio({h:ha,
s:1,v:1});ua.css("background-color",c.toHexString());c=ka;!(1>P)||0===P&&"name"===c||"hex"!==c&&"hex3"!==c&&"hex6"!==c&&"name"!==c||(c="rgb");var a=E({format:c}),b="";ea.removeClass("sp-clear-display");ea.css("background-color","transparent");if(!a&&Y)ea.addClass("sp-clear-display");else{var b=a.toHexString(),A=a.toRgbString();k||1===a.alpha?ea.css("background-color",A):(ea.css("background-color","transparent"),ea.css("filter",a.toFilter()));if(p.showAlpha){A=a.toRgb();A.a=0;var A=tinycolor(A).toRgbString(),
d="linear-gradient(left, "+A+", "+b+")";n?la.css("filter",tinycolor(A).toFilter({gradientType:1},b)):(la.css("background","-webkit-"+d),la.css("background","-moz-"+d),la.css("background","-ms-"+d),la.css("background","linear-gradient(to right, "+A+", "+b+")"))}b=a.toString(c)}p.showInput&&T.val(b);p.showPalette&&I();w()}function y(){var c=ia,a=ja;Y&&V?(va.hide(),wa.hide(),xa.hide()):(va.show(),wa.show(),xa.show(),c*=O,a=K-a*K,c=Math.max(-D,Math.min(O-D,c-D)),a=Math.max(-D,Math.min(K-D,a-D)),xa.css({top:a+
"px",left:c+"px"}),va.css({left:P*A-Oa/2+"px"}),wa.css({top:ha*sa-Pa+"px"}))}function H(c){var a=E(),b="",A=!tinycolor.equals(a,Z);a&&(b=a.toString(ka),fa(a));ya&&B.val(b);c&&A&&(C.change(a),B.trigger("change",[a]))}function J(){O=ua.width();K=ua.height();D=xa.height();Ea.width();sa=Ea.height();Pa=wa.height();A=Qa.width();Oa=va.width();M||(v.css("position","absolute"),p.offset?v.offset(p.offset):v.offset(f(v,ma)));y();p.showPalette&&I();B.trigger("reflow.spectrum")}function Fa(){aa();Ga=!0;B.attr("disabled",
!0);ma.addClass("sp-disabled")}var p=e(h,d),M=p.flat,R=p.showSelectionPalette,t=p.localStorageKey,F=p.theme,C=p.callbacks,ba=q(J,10),r=!1,O=0,K=0,D=0,sa=0,A=0,Oa=0,Pa=0,ha=0,ia=0,ja=0,P=1,qa=[],ga=[],ra={},N=p.selectionPalette.slice(0),Ua=p.maxSelectionSize,Na="sp-dragging",S=null,Ca=d.ownerDocument,B=c(d),Ga=!1,v=c(X,Ca).addClass(F),Wa=v.find(".sp-picker-container"),ua=v.find(".sp-color"),xa=v.find(".sp-dragger"),Ea=v.find(".sp-hue"),wa=v.find(".sp-slider"),la=v.find(".sp-alpha-inner"),Qa=v.find(".sp-alpha"),
va=v.find(".sp-alpha-handle"),T=v.find(".sp-input"),La=v.find(".sp-palette"),Ma=v.find(".sp-initial"),Ra=v.find(".sp-cancel"),Ha=v.find(".sp-clear"),Sa=v.find(".sp-choose"),Ba=v.find(".sp-palette-toggle"),ya=B.is("input"),Xa=ya&&x&&"color"===B.attr("type"),Ia=ya&&!M,ca=Ia?c(G).addClass(F).addClass(p.className).addClass(p.replacerClassName):c([]),ma=Ia?ca:B,ea=ca.find(".sp-preview-inner"),na=p.color||ya&&B.val(),Z=!1,Da=p.preferredFormat,ka=Da,Va=!p.showButtons||p.clickoutFiresChange,V=!na,Y=p.allowEmpty&&
!Xa;(function(){function a(b){b.data&&b.data.ignore?(U(c(b.target).closest(".sp-thumb-el").data("color")),da()):(U(c(b.target).closest(".sp-thumb-el").data("color")),da(),H(!0),p.hideAfterPaletteSelect&&aa());return!1}n&&v.find("*:not(input)").attr("unselectable","on");s();Ia&&B.after(ca).hide();Y||Ha.hide();if(M)B.after(v).hide();else{var b="parent"===p.appendTo?B.parent():c(p.appendTo);1!==b.length&&(b=c("body"));b.append(v)}m();ma.bind("click.spectrum touchstart.spectrum",function(a){Ga||ta();
a.stopPropagation();c(a.target).is("input")||a.preventDefault()});(B.is(":disabled")||!0===p.disabled)&&Fa();v.click(g);T.change(W);T.bind("paste",function(){setTimeout(W,1)});T.keydown(function(c){13==c.keyCode&&W()});Ra.text(p.cancelText);Ra.bind("click.spectrum",function(c){c.stopPropagation();c.preventDefault();U(Z,!0);aa()});Ha.attr("title",p.clearText);Ha.bind("click.spectrum",function(c){c.stopPropagation();c.preventDefault();V=!0;da();M&&H(!0)});Sa.text(p.chooseText);Sa.bind("click.spectrum",
function(c){c.stopPropagation();c.preventDefault();T.hasClass("sp-validation-error")||(H(!0),aa())});Ba.text(p.showPaletteOnly?p.togglePaletteMoreText:p.togglePaletteLessText);Ba.bind("click.spectrum",function(c){c.stopPropagation();c.preventDefault();p.showPaletteOnly=!p.showPaletteOnly;p.showPaletteOnly||M||v.css("left","-="+(Wa.outerWidth(!0)+5));s()});l(Qa,function(c,a,b){P=c/A;V=!1;b.shiftKey&&(P=Math.round(10*P)/10);da()},L,z);l(Ea,function(c,a){ha=parseFloat(a/sa);V=!1;p.showAlpha||(P=1);da()},
L,z);l(ua,function(c,a,b){b.shiftKey?S||(b=K-ja*K,S=Math.abs(c-ia*O)>Math.abs(a-b)?"x":"y"):S=null;b=!S||"y"===S;S&&"x"!==S||(ia=parseFloat(c/O));b&&(ja=parseFloat((K-a)/K));V=!1;p.showAlpha||(P=1);da()},L,z);na?(U(na),Q(),ka=Da||tinycolor(na).format,fa(na)):Q();M&&oa();b=n?"mousedown.spectrum":"click.spectrum touchstart.spectrum";La.delegate(".sp-thumb-el",b,a);Ma.delegate(".sp-thumb-el:nth-child(1)",b,{ignore:!0},a)})();var za={show:oa,hide:aa,toggle:ta,reflow:J,option:function(b,A){if(b===a)return c.extend({},
p);if(A===a)return p[b];p[b]=A;s()},enable:function(){Ga=!1;B.attr("disabled",!1);ma.removeClass("sp-disabled")},disable:Fa,offset:function(c){p.offset=c;J()},set:function(c){U(c);H()},get:E,destroy:function(){B.show();ma.unbind("click.spectrum touchstart.spectrum");v.remove();ca.remove();u[za.id]=null},container:v};za.id=u.push(za)-1;return za}function f(a,b){var d=a.outerWidth(),k=a.outerHeight(),e=b.outerHeight(),n=a[0].ownerDocument,f=n.documentElement,h=f.clientWidth+c(n).scrollLeft(),n=f.clientHeight+
c(n).scrollTop(),f=b.offset();f.top+=e;f.left-=Math.min(f.left,f.left+d>h&&h>d?Math.abs(f.left+d-h):0);f.top-=Math.min(f.top,f.top+k>n&&n>k?Math.abs(k+e-0):0);return f}function h(){}function g(c){c.stopPropagation()}function m(c,a){var b=Array.prototype.slice,d=b.call(arguments,2);return function(){return c.apply(a,d.concat(b.call(arguments)))}}function l(a,b,d,k){function e(c){c.stopPropagation&&c.stopPropagation();c.preventDefault&&c.preventDefault();c.returnValue=!1}function f(c){if(x){if(n&&9>
g.documentMode&&!c.button)return h();var d=c.originalEvent&&c.originalEvent.touches,k=d?d[0].pageY:c.pageY,d=Math.max(0,Math.min((d?d[0].pageX:c.pageX)-s.left,X)),k=Math.max(0,Math.min(k-s.top,m));q&&e(c);b.apply(a,[d,k,c])}}function h(){x&&(c(g).unbind(G),c(g.body).removeClass("sp-dragging"),k.apply(a,arguments));x=!1}b=b||function(){};d=d||function(){};k=k||function(){};var g=document,x=!1,s={},m=0,X=0,q="ontouchstart"in window,G={};G.selectstart=e;G.dragstart=e;G["touchmove mousemove"]=f;G["touchend mouseup"]=
h;c(a).bind("touchstart mousedown",function(b){(b.which?3==b.which:2==b.button)||x||!1===d.apply(a,arguments)||(x=!0,m=c(a).height(),X=c(a).width(),s=c(a).offset(),c(g).bind(G),c(g.body).addClass("sp-dragging"),q||f(b),e(b))})}function q(c,a,b){var d;return function(){var k=this,e=arguments,f=function(){d=null;c.apply(k,e)};b&&clearTimeout(d);if(b||!d)d=setTimeout(f,a)}}var s={beforeShow:h,move:h,change:h,show:h,hide:h,color:!1,flat:!1,showInput:!1,allowEmpty:!1,showButtons:!0,clickoutFiresChange:!1,
...
l;c.fn.spectrum.defaults=s;c.spectrum={};c.spectrum.localization={};c.spectrum.palettes={};c.fn.spectrum.processNativeColorInputs=function(){x||c("input[type=color]").spectrum({preferredFormat:"hex6"})};(function(){function c(a,b,d){a=y(a,255);b=y(b,255);d=y(d,255);var k=C(a,b,d),e=F(a,b,d),f,n=(k+e)/2;if(k==e)f=e=0;else{var h=k-e,e=0.5<n?h/(2-k-e):h/(k+e);switch(k){case a:f=(b-d)/h+(b<d?6:0);break;case b:f=(d-a)/h+2;break;case d:f=(a-b)/h+4}f/=6}return{h:f,s:e,l:n}}function a(c,b,d){function k(c,
a,b){0>b&&(b+=1);1<b&&(b-=1);return b<1/6?c+6*(a-c)*b:0.5>b?a:b<2/3?c+6*(a-c)*(2/3-b):c}c=y(c,360);b=y(b,100);d=y(d,100);if(0===b)d=b=c=d;else{var e=0.5>d?d*(1+b):d+b-d*b,f=2*d-e;d=k(f,e,c+1/3);b=k(f,e,c);c=k(f,e,c-1/3)}return{r:255*d,g:255*b,b:255*c}}function b(c,a,d){c=y(c,255);a=y(a,255);d=y(d,255);var k=C(c,a,d),e=F(c,a,d),f,n=k-e;if(k==e)f=0;else{switch(k){case c:f=(a-d)/n+(a<d?6:0);break;case a:f=(d-c)/n+2;break;case d:f=(c-a)/n+4}f/=6}return{h:f,s:0===k?0:n/k,v:k}}function d(c,a,b,k){c=[H(t(c).toString(16)),
H(t(a).toString(16)),H(t(b).toString(16))];return k&&c[0].charAt(0)==c[0].charAt(1)&&c[1].charAt(0)==c[1].charAt(1)&&c[2].charAt(0)==c[2].charAt(1)?c[0].charAt(0)+c[1].charAt(0)+c[2].charAt(0):c.join("")}function k(c,a,b,d){return[H(Math.round(255*parseFloat(d)).toString(16)),H(t(c).toString(16)),H(t(a).toString(16)),H(t(b).toString(16))].join("")}function e(c,a){a=0===a?0:a||10;var b=r(c).toHsl();b.s-=a/100;b.s=F(1,C(0,b.s));return r(b)}function f(c,a){a=0===a?0:a||10;var b=r(c).toHsl();b.s+=a/100;
b.s=F(1,C(0,b.s));return r(b)}function n(c){return r(c).desaturate(100)}function h(c,a){a=0===a?0:a||10;var b=r(c).toHsl();b.l+=a/100;b.l=F(1,C(0,b.l));return r(b)}function g(c,a){a=0===a?0:a||10;var b=r(c).toRgb();b.r=C(0,F(255,b.r-t(255*-(a/100))));b.g=C(0,F(255,b.g-t(255*-(a/100))));b.b=C(0,F(255,b.b-t(255*-(a/100))));return r(b)}function x(c,a){a=0===a?0:a||10;var b=r(c).toHsl();b.l-=a/100;b.l=F(1,C(0,b.l));return r(b)}function s(c,a){var b=r(c).toHsl(),d=(t(b.h)+a)%360;b.h=0>d?360+d:d;return r(b)}
function m(c){c=r(c).toHsl();c.h=(c.h+180)%360;return r(c)}function X(c){var a=r(c).toHsl(),b=a.h;return[r(c),r({h:(b+120)%360,s:a.s,l:a.l}),r({h:(b+240)%360,s:a.s,l:a.l})]}function q(c){var a=r(c).toHsl(),b=a.h;return[r(c),r({h:(b+90)%360,s:a.s,l:a.l}),r({h:(b+180)%360,s:a.s,l:a.l}),r({h:(b+270)%360,s:a.s,l:a.l})]}function G(c){var a=r(c).toHsl(),b=a.h;return[r(c),r({h:(b+72)%360,s:a.s,l:a.l}),r({h:(b+216)%360,s:a.s,l:a.l})]}function u(c,a,b){a=a||6;b=b||30;var d=r(c).toHsl();b=360/b;c=[r(c)];for(d.h=
(d.h-(b*a>>1)+720)%360;--a;)d.h=(d.h+b)%360,c.push(r(d));return c}function l(c,a){a=a||6;for(var b=r(c).toHsv(),d=b.h,k=b.s,b=b.v,e=[],f=1/a;a--;)e.push(r({h:d,s:k,v:b})),b=(b+f)%1;return e}function Q(c){c=parseFloat(c);if(isNaN(c)||0>c||1<c)c=1;return c}function y(c,a){"string"==typeof c&&(-1!=c.indexOf(".")&&1===parseFloat(c))&&(c="100%");var b="string"===typeof c&&-1!=c.indexOf("%");c=F(a,C(0,parseFloat(c)));b&&(c=parseInt(c*a,10)/100);return 1E-6>R.abs(c-a)?1:c%a/parseFloat(a)}function H(c){return 1==
c.length?"0"+c:""+c}function J(c){1>=c&&(c=100*c+"%");return c}var Fa=/^[\s,#]+/,p=/\s+$/,M=0,R=Math,t=R.round,F=R.min,C=R.max,ba=R.random,r=function A(c,b){var d,k,e,f,n;c=c?c:"";b=b||{};if(c instanceof A)return c;if(!(this instanceof A))return new A(c,b);k=c;n={r:0,g:0,b:0};var h=1;e=d=!1;if("string"==typeof k)a:{k=k.replace(Fa,"").replace(p,"").toLowerCase();var g=!1;if(O[k])k=O[k],g=!0;else if("transparent"==k){k={r:0,g:0,b:0,a:0,format:"name"};break a}k=(f=D.rgb.exec(k))?{r:f[1],g:f[2],b:f[3]}:
...
16),g:parseInt(f[2]+""+f[2],16),b:parseInt(f[3]+""+f[3],16),format:g?"name":"hex"}:!1}if("object"==typeof k){if(k.hasOwnProperty("r")&&k.hasOwnProperty("g")&&k.hasOwnProperty("b"))n={r:255*y(k.r,255),g:255*y(k.g,255),b:255*y(k.b,255)},d=!0,e="%"===String(k.r).substr(-1)?"prgb":"rgb";else if(k.hasOwnProperty("h")&&k.hasOwnProperty("s")&&k.hasOwnProperty("v")){k.s=J(k.s);k.v=J(k.v);e=k.h;g=k.s;n=k.v;e=6*y(e,360);g=y(g,100);n=y(n,100);d=R.floor(e);var x=e-d;e=n*(1-g);f=n*(1-x*g);g=n*(1-(1-x)*g);d%=6;
n={r:255*[n,f,e,e,g,n][d],g:255*[g,n,n,f,e,e][d],b:255*[e,e,g,n,n,f][d]};d=!0;e="hsv"}else k.hasOwnProperty("h")&&(k.hasOwnProperty("s")&&k.hasOwnProperty("l"))&&(k.s=J(k.s),k.l=J(k.l),n=a(k.h,k.s,k.l),d=!0,e="hsl");k.hasOwnProperty("a")&&(h=k.a)}h=Q(h);k=k.format||e;e=F(255,C(n.r,0));f=F(255,C(n.g,0));n=F(255,C(n.b,0));this._originalInput=c;this._r=e;this._g=f;this._b=n;this._a=h;this._roundA=t(100*this._a)/100;this._format=b.format||k;this._gradientType=b.gradientType;1>this._r&&(this._r=t(this._r));
1>this._g&&(this._g=t(this._g));1>this._b&&(this._b=t(this._b));this._ok=d;this._tc_id=M++};r.prototype={isDark:function(){return 128>this.getBrightness()},isLight:function(){return!this.isDark()},isValid:function(){return this._ok},getOriginalInput:function(){return this._originalInput},getFormat:function(){return this._format},getAlpha:function(){return this._a},getBrightness:function(){var c=this.toRgb();return(299*c.r+587*c.g+114*c.b)/1E3},setAlpha:function(c){this._a=Q(c);this._roundA=t(100*
...
arguments)},complement:function(){return this._applyCombination(m,arguments)},monochromatic:function(){return this._applyCombination(l,arguments)},splitcomplement:function(){return this._applyCombination(G,arguments)},triad:function(){return this._applyCombination(X,arguments)},tetrad:function(){return this._applyCombination(q,arguments)}};r.fromRatio=function(c,a){if("object"==typeof c){var b={},d;for(d in c)c.hasOwnProperty(d)&&(b[d]="a"===d?c[d]:J(c[d]));c=b}return r(c,a)};r.equals=function(c,
...
thistle:"d8bfd8",tomato:"ff6347",turquoise:"40e0d0",violet:"ee82ee",wheat:"f5deb3",white:"fff",whitesmoke:"f5f5f5",yellow:"ff0",yellowgreen:"9acd32"},K=r.hexNames=function(c){var a={},b;for(b in c)c.hasOwnProperty(b)&&(a[c[b]]=b);return a}(O),D={rgb:RegExp("rgb[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),rgba:RegExp("rgba[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),
...
h=e.WordArray=f.extend({init:function(c,b){c=this.words=c||[];this.sigBytes=b!=a?b:4*c.length},toString:function(c){return(c||m).stringify(this)},concat:function(c){var a=this.words,b=c.words,d=this.sigBytes;c=c.sigBytes;this.clamp();if(d%4)for(var f=0;f<c;f++)a[d+f>>>2]|=(b[f>>>2]>>>24-8*(f%4)&255)<<24-8*((d+f)%4);else if(65535<b.length)for(f=0;f<c;f+=4)a[d+f>>>2]=b[f>>>2];else a.push.apply(a,b);this.sigBytes+=c;return this},clamp:function(){var a=this.words,b=this.sigBytes;a[b>>>2]&=4294967295<<
32-8*(b%4);a.length=c.ceil(b/4)},clone:function(){var c=f.clone.call(this);c.words=this.words.slice(0);return c},random:function(a){for(var b=[],d=0;d<a;d+=4)b.push(4294967296*c.random()|0);return new h.init(b,a)}}),g=b.enc={},m=g.Hex={stringify:function(c){var a=c.words;c=c.sigBytes;for(var b=[],d=0;d<c;d++){var f=a[d>>>2]>>>24-8*(d%4)&255;b.push((f>>>4).toString(16));b.push((f&15).toString(16))}return b.join("")},parse:function(c){for(var a=c.length,b=[],d=0;d<a;d+=2)b[d>>>3]|=parseInt(c.substr(d,
2),16)<<24-4*(d%8);return new h.init(b,a/2)}},l=g.Latin1={stringify:function(c){var a=c.words;c=c.sigBytes;for(var b=[],d=0;d<c;d++)b.push(String.fromCharCode(a[d>>>2]>>>24-8*(d%4)&255));return b.join("")},parse:function(c){for(var a=c.length,b=[],d=0;d<a;d++)b[d>>>2]|=(c.charCodeAt(d)&255)<<24-8*(d%4);return new h.init(b,a)}},q=g.Utf8={stringify:function(c){try{return decodeURIComponent(escape(l.stringify(c)))}catch(a){throw Error("Malformed UTF-8 data");}},parse:function(c){return l.parse(unescape(encodeURIComponent(c)))}},
s=e.BufferedBlockAlgorithm=f.extend({reset:function(){this._data=new h.init;this._nDataBytes=0},_append:function(c){"string"==typeof c&&(c=q.parse(c));this._data.concat(c);this._nDataBytes+=c.sigBytes},_process:function(a){var b=this._data,d=b.words,f=b.sigBytes,e=this.blockSize,g=f/(4*e),g=a?c.ceil(g):c.max((g|0)-this._minBufferSize,0);a=g*e;f=c.min(4*a,f);if(a){for(var s=0;s<a;s+=e)this._doProcessBlock(d,s);s=d.splice(0,a);b.sigBytes-=f}return new h.init(s,f)},clone:function(){var c=f.clone.call(this);
...
this.receiveBuffer=[];this.sendBuffer=[];this.connected=!1;this.disconnected=!0;this.subEvents()}var f=a("socket.io-parser");e=a("component-emitter");var h=a("to-array"),g=a("./on"),m=a("component-bind"),l=a("debug")("socket.io-client:socket"),q=a("has-binary");a("indexof");b.exports=d;var s={connect:1,connect_error:1,connect_timeout:1,disconnect:1,error:1,reconnect:1,reconnect_attempt:1,reconnect_failed:1,reconnect_error:1,reconnecting:1},u=e.prototype.emit;e(d.prototype);d.prototype.subEvents=function(){var a=
this.io;this.subs=[g(a,"open",m(this,"onopen")),g(a,"packet",m(this,"onpacket")),g(a,"close",m(this,"onclose"))]};d.prototype.open=d.prototype.connect=function(){if(this.connected)return this;this.io.open();if("open"==this.io.readyState)this.onopen();return this};d.prototype.send=function(){var a=h(arguments);a.unshift("message");this.emit.apply(this,a);return this};d.prototype.emit=function(a){if(s.hasOwnProperty(a))return u.apply(this,arguments),this;var b=h(arguments),d=f.EVENT;q(b)&&(d=f.BINARY_EVENT);
...
b):this.receiveBuffer.push(b)};d.prototype.ack=function(a){var b=this,d=!1;return function(){if(!d){d=!0;var e=h(arguments);l("sending ack %j",e);var g=q(e)?f.BINARY_ACK:f.ACK;b.packet({type:g,id:a,data:e})}}};d.prototype.onack=function(a){l("calling ack %s with %j",a.id,a.data);this.acks[a.id].apply(this,a.data);delete this.acks[a.id]};d.prototype.onconnect=function(){this.connected=!0;this.disconnected=!1;this.emit("connect");this.emitBuffered()};d.prototype.emitBuffered=function(){var a;for(a=
...
(a=b.hostname+a),/^(https?|wss?):\/\//.test(a)||(e("protocol-less url %s",a),a="undefined"!=typeof b?b.protocol+"//"+a:"https://"+a),e("parse %s",a),l=f(a));l.port||(/^(http|ws)$/.test(l.protocol)?l.port="80":/^(http|ws)s$/.test(l.protocol)&&(l.port="443"));l.path=l.path||"/";l.id=l.protocol+"://"+l.host+":"+l.port;l.href=l.protocol+"://"+l.host+(b&&b.port==l.port?"":":"+l.port);return l}}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{debug:9,parseuri:38}],7:[function(a,
...
"websocket"];this.readyState="";this.writeBuffer=[];this.callbackBuffer=[];this.policyPort=b.policyPort||843;this.rememberUpgrade=b.rememberUpgrade||!1;this.open();this.binaryType=null;this.onlyBinaryUpgrades=b.onlyBinaryUpgrades}var h=a("./transports"),g=a("component-emitter"),m=a("debug")("engine.io-client:socket"),l=a("indexof"),q=a("engine.io-parser"),s=a("parseuri"),u=a("parsejson"),n=a("parseqs");b.exports=e;e.priorWebsocketSuccess=!1;g(e.prototype);e.protocol=q.protocol;e.Socket=e;e.Transport=
a("./transport");e.transports=a("./transports");e.parser=a("engine.io-parser");e.prototype.createTransport=function(a){m('creating transport "%s"',a);var b=this.query,d={},e;for(e in b)b.hasOwnProperty(e)&&(d[e]=b[e]);d.EIO=q.protocol;d.transport=a;this.id&&(d.sid=this.id);return new h[a]({agent:this.agent,hostname:this.hostname,port:this.port,secure:this.secure,path:this.path,query:d,forceJSONP:this.forceJSONP,jsonp:this.jsonp,forceBase64:this.forceBase64,enablesXDR:this.enablesXDR,timestampRequests:this.timestampRequests,
...
function(a){m("setting transport %s",a.name);var b=this;this.transport&&(m("clearing existing transport %s",this.transport.name),this.transport.removeAllListeners());this.transport=a;a.on("drain",function(){b.onDrain()}).on("packet",function(a){b.onPacket(a)}).on("error",function(a){b.onError(a)}).on("close",function(){b.onClose("transport close")})};e.prototype.probe=function(a){function b(){if(I.onlyBinaryUpgrades){var d=!this.supportsBinary&&I.transport.supportsBinary;u=u||d}u||(m('probe transport "%s" opened',
a),l.send([{type:"ping",data:"probe"}]),l.once("packet",function(b){u||("pong"==b.type&&"probe"==b.data?(m('probe transport "%s" pong',a),I.upgrading=!0,I.emit("upgrading",l),e.priorWebsocketSuccess="websocket"==l.name,m('pausing current transport "%s"',I.transport.name),I.transport.pause(function(){u||("closed"==I.readyState||"closing"==I.readyState)||(m("changing transport and sending upgrade packet"),s(),I.setTransport(l),l.send([{type:"upgrade"}]),I.emit("upgrade",l),l=null,I.upgrading=!1,I.flush())})):
(m('probe transport "%s" failed',a),b=Error("probe error"),b.transport=l.name,I.emit("upgradeError",b)))}))}function d(){u||(u=!0,s(),l.close(),l=null)}function h(b){var e=Error("probe error: "+b);e.transport=l.name;d();m('probe transport "%s" failed because of error: %s',a,b);I.emit("upgradeError",e)}function g(){h("transport closed")}function q(){h("socket closed")}function n(a){l&&a.name!=l.name&&(m('"%s" works - aborting "%s"',a.name,l.name),d())}function s(){l.removeListener("open",b);l.removeListener("error",
h);l.removeListener("close",g);I.removeListener("close",q);I.removeListener("upgrading",n)}m('probing transport "%s"',a);var l=this.createTransport(a,{probe:1}),u=!1,I=this;e.priorWebsocketSuccess=!1;l.once("open",b);l.once("error",h);l.once("close",g);this.once("close",q);this.once("upgrading",n);l.open()};e.prototype.onOpen=function(){m("socket open");this.readyState="open";e.priorWebsocketSuccess="websocket"==this.transport.name;this.emit("open");this.flush();if("open"==this.readyState&&this.upgrade&&
...
"engine.io-parser":21}],14:[function(a,b,e){(function(b){var f=a("xmlhttprequest"),h=a("./polling-xhr"),g=a("./polling-jsonp"),m=a("./websocket");e.polling=function(a){var e=!1,m=!1,u=!1!==a.jsonp;b.location&&(m="https:"==location.protocol,(e=location.port)||(e=m?443:80),e=a.hostname!=location.hostname||e!=a.port,m=a.secure!=m);a.xdomain=e;a.xscheme=m;if("open"in new f(a)&&!a.forceJSONP)return new h(a);if(!u)throw Error("JSONP disabled");return new g(a)};e.websocket=m}).call(this,"undefined"!==typeof self?
self:"undefined"!==typeof window?window:{})},{"./polling-jsonp":15,"./polling-xhr":16,"./websocket":18,xmlhttprequest:19}],15:[function(a,b,e){(function(d){function e(){}function h(a){g.call(this,a);this.query=this.query||{};s||(d.___eio||(d.___eio=[]),s=d.___eio);this.index=s.length;var b=this;s.push(function(a){b.onData(a)});this.query.j=this.index;d.document&&d.addEventListener&&d.addEventListener("beforeunload",function(){b.script&&(b.script.onerror=e)})}var g=a("./polling"),m=a("component-inherit");
b.exports=h;var l=/\n/g,q=/\\n/g,s;m(h,g);h.prototype.supportsBinary=!1;h.prototype.doClose=function(){this.script&&(this.script.parentNode.removeChild(this.script),this.script=null);this.form&&(this.form.parentNode.removeChild(this.form),this.form=null);g.prototype.doClose.call(this)};h.prototype.doPoll=function(){var a=this,b=document.createElement("script");this.script&&(this.script.parentNode.removeChild(this.script),this.script=null);b.async=!0;b.src=this.uri();b.onerror=function(b){a.onError("jsonp poll error",
b)};var d=document.getElementsByTagName("script")[0];d.parentNode.insertBefore(b,d);this.script=b;"undefined"!=typeof navigator&&/gecko/i.test(navigator.userAgent)&&setTimeout(function(){var a=document.createElement("iframe");document.body.appendChild(a);document.body.removeChild(a)},100)};h.prototype.doWrite=function(a,b){function d(){e();b()}function e(){if(f.iframe)try{f.form.removeChild(f.iframe)}catch(a){f.onError("jsonp polling iframe removal error",a)}try{s=document.createElement('<iframe src="javascript:0" name="'+
f.iframeId+'">')}catch(b){s=document.createElement("iframe"),s.name=f.iframeId,s.src="javascript:0"}s.id=f.iframeId;f.form.appendChild(s);f.iframe=s}var f=this;if(!this.form){var h=document.createElement("form"),g=document.createElement("textarea"),m=this.iframeId="eio_iframe_"+this.index,s;h.className="socketio";h.style.position="absolute";h.style.top="-1000px";h.style.left="-1000px";h.target=m;h.method="POST";h.setAttribute("accept-charset","utf-8");g.name="d";h.appendChild(g);document.body.appendChild(h);
this.form=h;this.area=g}this.form.action=this.uri();e();a=a.replace(q,"\\\n");this.area.value=a.replace(l,"\\n");try{this.form.submit()}catch(Ta){}this.iframe.attachEvent?this.iframe.onreadystatechange=function(){"complete"==f.iframe.readyState&&d()}:this.iframe.onload=d}}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{"./polling":17,"component-inherit":20}],16:[function(a,b,e){(function(d){function e(){}function h(a){q.call(this,a);if(d.location){var b="https:"==
location.protocol,e=location.port;e||(e=b?443:80);this.xd=a.hostname!=d.location.hostname||e!=a.port;this.xs=a.secure!=b}}function g(a){this.method=a.method||"GET";this.uri=a.uri;this.xd=!!a.xd;this.xs=!!a.xs;this.async=!1!==a.async;this.data=void 0!=a.data?a.data:null;this.agent=a.agent;this.isBinary=a.isBinary;this.supportsBinary=a.supportsBinary;this.enablesXDR=a.enablesXDR;this.create()}function m(){for(var a in g.requests)g.requests.hasOwnProperty(a)&&g.requests[a].abort()}var l=a("xmlhttprequest"),
q=a("./polling"),s=a("component-emitter"),u=a("component-inherit"),n=a("debug")("engine.io-client:polling-xhr");b.exports=h;b.exports.Request=g;u(h,q);h.prototype.supportsBinary=!0;h.prototype.request=function(a){a=a||{};a.uri=this.uri();a.xd=this.xd;a.xs=this.xs;a.agent=this.agent||!1;a.supportsBinary=this.supportsBinary;a.enablesXDR=this.enablesXDR;return new g(a)};h.prototype.doWrite=function(a,b){var d=this.request({method:"POST",data:a,isBinary:"string"!==typeof a&&void 0!==a}),e=this;d.on("success",
b);d.on("error",function(a){e.onError("xhr post error",a)});this.sendXhr=d};h.prototype.doPoll=function(){n("xhr poll");var a=this.request(),b=this;a.on("data",function(a){b.onData(a)});a.on("error",function(a){b.onError("xhr poll error",a)});this.pollXhr=a};s(g.prototype);g.prototype.create=function(){var a=this.xhr=new l({agent:this.agent,xdomain:this.xd,xscheme:this.xs,enablesXDR:this.enablesXDR}),b=this;try{n("xhr open %s: %s",this.method,this.uri);a.open(this.method,this.uri,this.async);this.supportsBinary&&
...
(g.requestsCount=0,g.requests={},d.attachEvent?d.attachEvent("onunload",m):d.addEventListener&&d.addEventListener("beforeunload",m))}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{"./polling":17,"component-emitter":8,"component-inherit":20,debug:9,xmlhttprequest:19}],17:[function(a,b,e){function d(a){var b=a&&a.forceBase64;if(!l||b)this.supportsBinary=!1;f.call(this,a)}var f=a("../transport"),h=a("parseqs"),g=a("engine.io-parser");e=a("component-inherit");var m=
...
b,!0,d)};return f.readAsArrayBuffer(a.data)}function h(a,b,d){var e=Array(a.length);d=q(a.length,d);for(var f=function(a,d,f){b(d,function(b,d){e[a]=d;f(b,e)})},h=0;h<a.length;h++)f(h,a[h],d)}var g=a("./keys"),m=a("arraybuffer.slice"),l=a("base64-arraybuffer"),q=a("after"),s=a("utf8"),u=navigator.userAgent.match(/Android/i);e.protocol=3;var n=e.packets={open:0,close:1,ping:2,pong:3,message:4,upgrade:5,noop:6},k=g(n),x={type:"error",data:"parser error"},G=a("blob");e.encodePacket=function(a,h,g,k){"function"==
typeof h&&(k=h,h=!1);"function"==typeof g&&(k=g,g=null);var m=void 0===a.data?void 0:a.data.buffer||a.data;if(b.ArrayBuffer&&m instanceof ArrayBuffer){if(h){g=a.data;h=new Uint8Array(g);g=new Uint8Array(1+g.byteLength);g[0]=n[a.type];for(a=0;a<h.length;a++)g[a+1]=h[a];a=k(g.buffer)}else a=e.encodeBase64Packet(a,k);return a}if(G&&m instanceof b.Blob)return h?u?a=f(a,h,k):(h=new Uint8Array(1),h[0]=n[a.type],a=new G([h.buffer,a.data]),a=k(a)):a=e.encodeBase64Packet(a,k),a;h=n[a.type];void 0!==a.data&&
(h+=g?s.encode(String(a.data)):String(a.data));return k(""+h)};e.encodeBase64Packet=function(a,f){var h="b"+e.packets[a.type];if(G&&a.data instanceof G){var g=new FileReader;g.onload=function(){var a=g.result.split(",")[1];f(h+a)};return g.readAsDataURL(a.data)}var k;try{k=String.fromCharCode.apply(null,new Uint8Array(a.data))}catch(m){k=new Uint8Array(a.data);for(var l=Array(k.length),n=0;n<k.length;n++)l[n]=k[n];k=String.fromCharCode.apply(null,l)}h+=b.btoa(k);return f(h)};e.decodePacket=function(a,
b,d){if("string"==typeof a||void 0===a){if("b"==a.charAt(0))return e.decodeBase64Packet(a.substr(1),b);if(d)try{a=s.decode(a)}catch(f){return x}d=a.charAt(0);return Number(d)==d&&k[d]?1<a.length?{type:k[d],data:a.substring(1)}:{type:k[d]}:x}d=(new Uint8Array(a))[0];a=m(a,1);G&&"blob"===b&&(a=new G([a]));return{type:k[d],data:a}};e.decodeBase64Packet=function(a,e){var f=k[a.charAt(0)];if(!b.ArrayBuffer)return{type:f,data:{base64:!0,data:a.substr(1)}};var h=l.decode(a.substr(1));"blob"===e&&G&&(h=new G([h]));
return{type:f,data:h}};e.encodePayload=function(a,b,d){"function"==typeof b&&(d=b,b=null);if(b)return G&&!u?e.encodePayloadAsBlob(a,d):e.encodePayloadAsArrayBuffer(a,d);if(!a.length)return d("0:");h(a,function(a,d){e.encodePacket(a,b,!0,function(a){d(null,a.length+":"+a)})},function(a,b){return d(b.join(""))})};e.decodePayload=function(a,b,d){if("string"!=typeof a)return e.decodePayloadAsBinary(a,b,d);"function"===typeof b&&(d=b,b=null);var f;if(""==a)return d(x,0,1);f="";for(var h,g,k=0,m=a.length;k<
m;k++)if(g=a.charAt(k),":"!=g)f+=g;else{if(""==f||f!=(h=Number(f)))return d(x,0,1);g=a.substr(k+1,h);if(f!=g.length)return d(x,0,1);if(g.length){f=e.decodePacket(g,b,!0);if(x.type==f.type&&x.data==f.data)return d(x,0,1);if(!1===d(f,k+h,m))return}k+=h;f=""}if(""!=f)return d(x,0,1)};e.encodePayloadAsArrayBuffer=function(a,b){if(!a.length)return b(new ArrayBuffer(0));h(a,function(a,b){e.encodePacket(a,!0,!0,function(a){return b(null,a)})},function(a,d){var e=d.reduce(function(a,b){var d;d="string"===
typeof b?b.length:b.byteLength;return a+d.toString().length+d+2},0),f=new Uint8Array(e),h=0;d.forEach(function(a){var b="string"===typeof a,d=a;if(b){for(var d=new Uint8Array(a.length),e=0;e<a.length;e++)d[e]=a.charCodeAt(e);d=d.buffer}b?f[h++]=0:f[h++]=1;a=d.byteLength.toString();for(e=0;e<a.length;e++)f[h++]=parseInt(a[e]);f[h++]=255;d=new Uint8Array(d);for(e=0;e<d.length;e++)f[h++]=d[e]});return b(f.buffer)})};e.encodePayloadAsBlob=function(a,b){h(a,function(a,b){e.encodePacket(a,!0,!0,function(a){var d=
new Uint8Array(1);d[0]=1;if("string"===typeof a){for(var e=new Uint8Array(a.length),f=0;f<a.length;f++)e[f]=a.charCodeAt(f);a=e.buffer;d[0]=0}for(var e=(a instanceof ArrayBuffer?a.byteLength:a.size).toString(),h=new Uint8Array(e.length+1),f=0;f<e.length;f++)h[f]=parseInt(e[f]);h[e.length]=255;G&&(a=new G([d.buffer,h.buffer,a]),b(null,a))})},function(a,d){return b(new G(d))})};e.decodePayloadAsBinary=function(a,b,d){"function"===typeof b&&(d=b,b=null);for(var f=[],h=!1;0<a.byteLength;){for(var g=new Uint8Array(a),
k=0===g[0],l="",n=1;255!=g[n];n++){if(310<l.length){h=!0;break}l+=g[n]}if(h)return d(x,0,1);a=m(a,2+l.length);l=parseInt(l);g=m(a,0,l);if(k)try{g=String.fromCharCode.apply(null,new Uint8Array(g))}catch(s){for(k=new Uint8Array(g),g="",n=0;n<k.length;n++)g+=String.fromCharCode(k[n])}f.push(g);a=m(a,l)}var q=f.length;f.forEach(function(a,f){d(e.decodePacket(a,b,!0),f,q)})}}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{"./keys":22,after:23,"arraybuffer.slice":24,
...
e=e||g;if(a.slice)return a.slice(b,e);0>b&&(b+=g);0>e&&(e+=g);e>g&&(e=g);if(b>=g||b>=e||0===g)return new ArrayBuffer(0);a=new Uint8Array(a);for(var g=new Uint8Array(e-b),m=0;b<e;b++,m++)g[m]=a[b];return g.buffer}},{}],25:[function(a,b,e){(function(a){e.encode=function(b){b=new Uint8Array(b);var e,g=b.length,m="";for(e=0;e<g;e+=3)m+=a[b[e]>>2],m+=a[(b[e]&3)<<4|b[e+1]>>4],m+=a[(b[e+1]&15)<<2|b[e+2]>>6],m+=a[b[e+2]&63];2===g%3?m=m.substring(0,m.length-1)+"=":1===g%3&&(m=m.substring(0,m.length-2)+"==");
return m};e.decode=function(b){var e=0.75*b.length,g=b.length,m=0,l,q,s,u;"="===b[b.length-1]&&(e--,"="===b[b.length-2]&&e--);for(var n=new ArrayBuffer(e),k=new Uint8Array(n),e=0;e<g;e+=4)l=a.indexOf(b[e]),q=a.indexOf(b[e+1]),s=a.indexOf(b[e+2]),u=a.indexOf(b[e+3]),k[m++]=l<<2|q>>4,k[m++]=(q&15)<<4|s>>2,k[m++]=(s&3)<<6|u&63;return n}})("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/")},{}],26:[function(a,b,e){(function(a){function e(a,b){b=b||{};for(var d=new h,f=0;f<a.length;f++)d.append(a[f]);
return b.type?d.getBlob(b.type):d.getBlob()}var h=a.BlobBuilder||a.WebKitBlobBuilder||a.MSBlobBuilder||a.MozBlobBuilder,g;try{g=2==(new Blob(["hi"])).size}catch(m){g=!1}var l=h&&h.prototype.append&&h.prototype.getBlob;a=g?a.Blob:l?e:void 0;b.exports=a}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{}],27:[function(a,b,e){(function(a){(function(f){function h(a){for(var b=[],d=0,e=a.length,f,g;d<e;)f=a.charCodeAt(d++),55296<=f&&56319>=f&&d<e?(g=a.charCodeAt(d++),
56320==(g&64512)?b.push(((f&1023)<<10)+(g&1023)+65536):(b.push(f),d--)):b.push(f);return b}function g(){if(x>=k)throw Error("Invalid byte index");var a=n[x]&255;x++;if(128==(a&192))return a&63;throw Error("Invalid continuation byte");}function m(){var a,b,d,e;if(x>k)throw Error("Invalid byte index");if(x==k)return!1;a=n[x]&255;x++;if(0==(a&128))return a;if(192==(a&224)){b=g();a=(a&31)<<6|b;if(128<=a)return a;throw Error("Invalid continuation byte");}if(224==(a&240)){b=g();d=g();a=(a&15)<<12|b<<6|
d;if(2048<=a)return a;throw Error("Invalid continuation byte");}if(240==(a&248)&&(b=g(),d=g(),e=g(),a=(a&15)<<18|b<<12|d<<6|e,65536<=a&&1114111>=a))return a;throw Error("Invalid UTF-8 detected");}var l="object"==typeof e&&e,q="object"==typeof b&&b&&b.exports==l&&b,s="object"==typeof a&&a;if(s.global===s||s.window===s)f=s;var u=String.fromCharCode,n,k,x,s={version:"2.0.0",encode:function(a){a=h(a);for(var b=a.length,d=-1,e,f="";++d<b;){e=a[d];if(0==(e&4294967168))e=u(e);else{var g="";0==(e&4294965248)?
g=u(e>>6&31|192):0==(e&4294901760)?(g=u(e>>12&15|224),g+=u(e>>6&63|128)):0==(e&4292870144)&&(g=u(e>>18&7|240),g+=u(e>>12&63|128),g+=u(e>>6&63|128));e=g+=u(e&63|128)}f+=e}return f},decode:function(a){n=h(a);k=n.length;x=0;a=[];for(var b;!1!==(b=m());)a.push(b);b=a.length;for(var d=-1,e,f="";++d<b;)e=a[d],65535<e&&(e-=65536,f+=u(e>>>10&1023|55296),e=56320|e&1023),f+=u(e);return f}};if(l&&!l.nodeType)if(q)q.exports=s;else{f={}.hasOwnProperty;for(var G in s)f.call(s,G)&&(l[G]=s[G])}else f.utf8=s})(this)}).call(this,
"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{}],28:[function(a,b,e){(function(a){var e=/^[\],:{}\s]*$/,h=/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,g=/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,m=/(?:^|:|,)(?:\s*\[)+/g,l=/^\s+/,q=/\s+$/;b.exports=function(b){if("string"!=typeof b||!b)return null;b=b.replace(l,"").replace(q,"");if(a.JSON&&JSON.parse)return JSON.parse(b);if(e.test(b.replace(h,"@").replace(g,"]").replace(m,"")))return(new Function("return "+
b))()}}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{}],29:[function(a,b,e){e.encode=function(a){var b="",e;for(e in a)a.hasOwnProperty(e)&&(b.length&&(b+="&"),b+=encodeURIComponent(e)+"="+encodeURIComponent(a[e]));return b};e.decode=function(a){var b={};a=a.split("&");for(var e=0,g=a.length;e<g;e++){var m=a[e].split("=");b[decodeURIComponent(m[0])]=decodeURIComponent(m[1])}return b}},{}],30:[function(a,b,e){var d=/^(?:(?![^:@]+:[^:@\/]*@)(http|https|ws|wss):\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?((?:[a-f0-9]{0,4}:){2,7}[a-f0-9]{0,4}|[^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/,
...
"";return b}},{}],39:[function(a,b,e){(function(b){var f=a("isarray"),h=a("./is-buffer");e.deconstructPacket=function(a){function b(a){if(!a)return a;if(h(a)){var e={_placeholder:!0,num:d.length};d.push(a);return e}if(f(a)){for(var e=Array(a.length),g=0;g<a.length;g++)e[g]=b(a[g]);return e}if("object"==typeof a&&!(a instanceof Date)){e={};for(g in a)e[g]=b(a[g]);return e}return a}var d=[];a.data=b(a.data);a.attachments=d.length;return{packet:a,buffers:d}};e.reconstructPacket=function(a,b){function d(a){if(a&&
a._placeholder)return b[a.num];if(f(a))for(var e=0;e<a.length;e++)a[e]=d(a[e]);else if(a&&"object"==typeof a)for(e in a)a[e]=d(a[e]);return a}a.data=d(a.data);a.attachments=void 0;return a};e.removeBlobs=function(a,e){function l(a,g,k){if(!a)return a;if(b.Blob&&a instanceof Blob||b.File&&a instanceof File){q++;var x=new FileReader;x.onload=function(){k?k[g]=this.result:s=this.result;--q||e(s)};x.readAsArrayBuffer(a)}else if(f(a))for(x=0;x<a.length;x++)l(a[x],x,a);else if(a&&"object"==typeof a&&!h(a))for(x in a)l(a[x],
x,a)}var q=0,s=a;l(s);q||e(s)}}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{"./is-buffer":41,isarray:42}],40:[function(a,b,e){function d(){}function f(a){var b,d=!1;b=""+a.type;if(e.BINARY_EVENT==a.type||e.BINARY_ACK==a.type)b+=a.attachments,b+="-";a.nsp&&"/"!=a.nsp&&(d=!0,b+=a.nsp);null!=a.id&&(d&&(b+=",",d=!1),b+=a.id);null!=a.data&&(d&&(b+=","),b+=q.stringify(a.data));l("encoded %j as %s",a,b);return b}function h(a,b){s.removeBlobs(a,function(a){var d=s.deconstructPacket(a);
a=f(d.packet);d=d.buffers;d.unshift(a);b(d)})}function g(){this.reconstructor=null}function m(a){this.reconPack=a;this.buffers=[]}var l=a("debug")("socket.io-parser"),q=a("json3");a("isarray");b=a("component-emitter");var s=a("./binary"),u=a("./is-buffer");e.protocol=4;e.types="CONNECT DISCONNECT EVENT BINARY_EVENT ACK BINARY_ACK ERROR".split(" ");e.CONNECT=0;e.DISCONNECT=1;e.EVENT=2;e.ACK=3;e.ERROR=4;e.BINARY_EVENT=5;e.BINARY_ACK=6;e.Encoder=d;e.Decoder=g;d.prototype.encode=function(a,b){l("encoding packet %j",
a);if(e.BINARY_EVENT==a.type||e.BINARY_ACK==a.type)h(a,b);else{var d=f(a);b([d])}};b(g.prototype);g.prototype.add=function(a){if("string"==typeof a){a:{var b={},d=0;b.type=Number(a.charAt(0));if(null==e.types[b.type])a={type:e.ERROR,data:"parser error"};else{if(e.BINARY_EVENT==b.type||e.BINARY_ACK==b.type){for(b.attachments="";"-"!=a.charAt(++d);)b.attachments+=a.charAt(d);b.attachments=Number(b.attachments)}if("/"==a.charAt(d+1))for(b.nsp="";++d;){var f=a.charAt(d);if(","==f)break;b.nsp+=f;if(d+
1==a.length)break}else b.nsp="/";f=a.charAt(d+1);if(""!=f&&Number(f)==f){for(b.id="";++d;){f=a.charAt(d);if(null==f||Number(f)!=f){--d;break}b.id+=a.charAt(d);if(d+1==a.length)break}b.id=Number(b.id)}if(a.charAt(++d))try{b.data=q.parse(a.substr(d))}catch(g){a={type:e.ERROR,data:"parser error"};break a}l("decoded %s as %j",a,b);a=b}}e.BINARY_EVENT==a.type||e.BINARY_ACK==a.type?(this.reconstructor=new m(a),0==this.reconstructor.reconPack.attachments&&this.emit("decoded",a)):this.emit("decoded",a)}else if(u(a)||
...
var d;if("bug-string-char-index"==a)d="a"!="a"[0];else if("json"==a)d=b("json-stringify")&&b("json-parse");else{var e;if("json-stringify"==a){d=s.stringify;var g="function"==typeof d&&u;if(g){(e=function(){return 1}).toJSON=e;try{g="0"===d(0)&&"0"===d(new Number)&&'""'==d(new String)&&d(h)===l&&d(l)===l&&d()===l&&"1"===d(e)&&"[1]"==d([e])&&"[null]"==d([l])&&"null"==d(null)&&"[null,null,null]"==d([l,h,null])&&'{"a":[1,true,false,null,"\\u0000\\b\\n\\f\\r\\t"]}'==d({a:[e,!0,!1,null,"\x00\b\n\f\r\t"]})&&
"1"===d(null,e)&&"[\n 1,\n 2\n]"==d([1,2],null,1)&&'"-271821-04-20T00:00:00.000Z"'==d(new Date(-864E13))&&'"+275760-09-13T00:00:00.000Z"'==d(new Date(864E13))&&'"-000001-01-01T00:00:00.000Z"'==d(new Date(-621987552E5))&&'"1969-12-31T23:59:59.999Z"'==d(new Date(-1))}catch(k){g=!1}}d=g}if("json-parse"==a){d=s.parse;if("function"==typeof d)try{if(0===d("0")&&!d(!1)){e=d('{"a":[1,true,false,null,"\\u0000\\b\\n\\f\\r\\t"]}');var m=5==e.a.length&&1===e.a[0];if(m){try{m=!d('"\t"')}catch(H){}if(m)try{m=1!==
d("01")}catch(n){}if(m)try{m=1!==d("1.")}catch(q){}}}}catch(p){m=!1}d=m}}return b[a]=!!d}var h={}.toString,g,m,l,q="object"==typeof JSON&&JSON,s="object"==typeof e&&e&&!e.nodeType&&e;s&&q?(s.stringify=q.stringify,s.parse=q.parse):s=a.JSON=q||{};var u=new Date(-0xc782b5b800cec);try{u=-109252==u.getUTCFullYear()&&0===u.getUTCMonth()&&1===u.getUTCDate()&&10==u.getUTCHours()&&37==u.getUTCMinutes()&&6==u.getUTCSeconds()&&708==u.getUTCMilliseconds()}catch(n){}if(!b("json")){var k=b("bug-string-char-index");
if(!u)var x=Math.floor,G=[0,31,59,90,120,151,181,212,243,273,304,334],X=function(a,b){return G[b]+365*(a-1970)+x((a-1969+(b=+(1<b)))/4)-x((a-1901+b)/100)+x((a-1601+b)/400)};(g={}.hasOwnProperty)||(g=function(a){var b={},d;(b.__proto__=null,b.__proto__={toString:1},b).toString!=h?g=function(a){var b=this.__proto__;a=a in(this.__proto__=null,this);this.__proto__=b;return a}:(d=b.constructor,g=function(a){var b=(this.constructor||d).prototype;return a in this&&!(a in b&&this[a]===b[a])});b=null;return g.call(this,
a)});var Ja={"boolean":1,number:1,string:1,undefined:1};m=function(a,b){var d=0,e,f,k;(e=function(){this.valueOf=0}).prototype.valueOf=0;f=new e;for(k in f)g.call(f,k)&&d++;e=f=null;d?m=2==d?function(a,b){var d={},e="[object Function]"==h.call(a),f;for(f in a)e&&"prototype"==f||(g.call(d,f)||!(d[f]=1)||!g.call(a,f))||b(f)}:function(a,b){var d="[object Function]"==h.call(a),e,f;for(e in a)d&&"prototype"==e||(!g.call(a,e)||(f="constructor"===e))||b(e);(f||g.call(a,e="constructor"))&&b(e)}:(f="valueOf toString toLocaleString propertyIsEnumerable isPrototypeOf hasOwnProperty constructor".split(" "),
m=function(a,b){var d="[object Function]"==h.call(a),e,k;if(k=!d)if(k="function"!=typeof a.constructor)k=typeof a.hasOwnProperty,k="object"==k?!!a.hasOwnProperty:!Ja[k];k=k?a.hasOwnProperty:g;for(e in a)d&&"prototype"==e||!k.call(a,e)||b(e);for(d=f.length;e=f[--d];k.call(a,e)&&b(e));});return m(a,b)};if(!b("json-stringify")){var Ya={92:"\\\\",34:'\\"',8:"\\b",12:"\\f",10:"\\n",13:"\\r",9:"\\t"},pa=function(a,b){return("000000"+(b||0)).slice(-a)},Ta=function(a){var b='"',d=0,e=a.length,f=10<e&&k,g;
for(f&&(g=a.split(""));d<e;d++){var h=a.charCodeAt(d);switch(h){case 8:case 9:case 10:case 12:case 13:case 34:case 92:b+=Ya[h];break;default:if(32>h){b+="\\u00"+pa(2,h.toString(16));break}b+=f?g[d]:k?a.charAt(d):a[d]}}return b+'"'},fa=function(a,b,d,e,f,k,s){var n,q,p,w,u,t,I,Ja,z;try{n=b[a]}catch(r){}if("object"==typeof n&&n)if(q=h.call(n),"[object Date]"!=q||g.call(n,"toJSON"))"function"==typeof n.toJSON&&("[object Number]"!=q&&"[object String]"!=q&&"[object Array]"!=q||g.call(n,"toJSON"))&&(n=
n.toJSON(a));else if(n>-1/0&&n<1/0){if(X){w=x(n/864E5);for(q=x(w/365.2425)+1970-1;X(q+1,0)<=w;q++);for(p=x((w-X(q,0))/30.42);X(q,p+1)<=w;p++);w=1+w-X(q,p);u=(n%864E5+864E5)%864E5;t=x(u/36E5)%24;I=x(u/6E4)%60;Ja=x(u/1E3)%60;u%=1E3}else q=n.getUTCFullYear(),p=n.getUTCMonth(),w=n.getUTCDate(),t=n.getUTCHours(),I=n.getUTCMinutes(),Ja=n.getUTCSeconds(),u=n.getUTCMilliseconds();n=(0>=q||1E4<=q?(0>q?"-":"+")+pa(6,0>q?-q:q):pa(4,q))+"-"+pa(2,p+1)+"-"+pa(2,w)+"T"+pa(2,t)+":"+pa(2,I)+":"+pa(2,Ja)+"."+pa(3,
u)+"Z"}else n=null;d&&(n=d.call(b,a,n));if(null===n)return"null";q=h.call(n);if("[object Boolean]"==q)return""+n;if("[object Number]"==q)return n>-1/0&&n<1/0?""+n:"null";if("[object String]"==q)return Ta(""+n);if("object"==typeof n){for(a=s.length;a--;)if(s[a]===n)throw TypeError();s.push(n);z=[];b=k;k+=f;if("[object Array]"==q){p=0;for(a=n.length;p<a;p++)q=fa(p,n,d,e,f,k,s),z.push(q===l?"null":q);a=z.length?f?"[\n"+k+z.join(",\n"+k)+"\n"+b+"]":"["+z.join(",")+"]":"[]"}else m(e||n,function(a){var b=
fa(a,n,d,e,f,k,s);b!==l&&z.push(Ta(a)+":"+(f?" ":"")+b)}),a=z.length?f?"{\n"+k+z.join(",\n"+k)+"\n"+b+"}":"{"+z.join(",")+"}":"{}";s.pop();return a}};s.stringify=function(a,b,d){var e,f,g,k;if("function"==typeof b||"object"==typeof b&&b)if("[object Function]"==(k=h.call(b)))f=b;else if("[object Array]"==k){g={};for(var l=0,m=b.length,p;l<m;p=b[l++],(k=h.call(p),"[object String]"==k||"[object Number]"==k)&&(g[p]=1));}if(d)if("[object Number]"==(k=h.call(d))){if(0<(d-=d%1))for(e="",10<d&&(d=10);e.length<
d;e+=" ");}else"[object String]"==k&&(e=10>=d.length?d:d.slice(0,10));return fa("",(p={},p[""]=a,p),f,g,e,"",[])}}if(!b("json-parse")){var Ka=String.fromCharCode,I={92:"\\",34:'"',47:"/",98:"\b",116:"\t",110:"\n",102:"\f",114:"\r"},w,L,z=function(){w=L=null;throw SyntaxError();},W=function(){for(var a=L,b=a.length,d,e,f,g,h;w<b;)switch(h=a.charCodeAt(w),h){case 9:case 10:case 13:case 32:w++;break;case 123:case 125:case 91:case 93:case 58:case 44:return d=k?a.charAt(w):a[w],w++,d;case 34:d="@";for(w++;w<
b;)if(h=a.charCodeAt(w),32>h)z();else if(92==h)switch(h=a.charCodeAt(++w),h){case 92:case 34:case 47:case 98:case 116:case 110:case 102:case 114:d+=I[h];w++;break;case 117:e=++w;for(f=w+4;w<f;w++)h=a.charCodeAt(w),48<=h&&57>=h||(97<=h&&102>=h||65<=h&&70>=h)||z();d+=Ka("0x"+a.slice(e,w));break;default:z()}else{if(34==h)break;h=a.charCodeAt(w);for(e=w;32<=h&&92!=h&&34!=h;)h=a.charCodeAt(++w);d+=a.slice(e,w)}if(34==a.charCodeAt(w))return w++,d;z();default:e=w;45==h&&(g=!0,h=a.charCodeAt(++w));if(48<=
...
ta=function(a){var b,d;"$"==a&&z();if("string"==typeof a){if("@"==(k?a.charAt(0):a[0]))return a.slice(1);if("["==a){for(b=[];;d||(d=!0)){a=W();if("]"==a)break;d&&(","==a?(a=W(),"]"==a&&z()):z());","==a&&z();b.push(ta(a))}return b}if("{"==a){for(b={};;d||(d=!0)){a=W();if("}"==a)break;d&&(","==a?(a=W(),"}"==a&&z()):z());","!=a&&"string"==typeof a&&"@"==(k?a.charAt(0):a[0])&&":"==W()||z();b[a.slice(1)]=ta(W())}return b}z()}return a},oa=function(a,b,d){d=Aa(a,b,d);d===l?delete a[b]:a[b]=d},Aa=function(a,
b,d){var e=a[b],f;if("object"==typeof e&&e)if("[object Array]"==h.call(e))for(f=e.length;f--;)oa(e,f,d);else m(e,function(a){oa(e,a,d)});return d.call(a,b,e)};s.parse=function(a,b){var d,e;w=0;L=""+a;d=ta(W());"$"!=W()&&z();w=L=null;return b&&"[object Function]"==h.call(b)?Aa((e={},e[""]=d,e),"",b):d}}}})(this)},{}],44:[function(a,b,e){b.exports=function(a,b){for(var e=[],g=(b=b||0)||0;g<a.length;g++)e[g-b]=a[g];return e}},{}]},{},[1])(1)});(function(c){"function"===typeof define&&define.amd?define(["jquery"],c):"object"==typeof exports&&"object"==typeof module?module.exports=c:c(jQuery)})(function(c,a){function b(a,b,d,e){for(var f=[],g=0;g<a.length;g++){var h=a[g];if(h){var l=tinycolor(h),m=0.5>l.toHsl().l?"sp-thumb-el sp-thumb-dark":"sp-thumb-el sp-thumb-light",m=m+(tinycolor.equals(b,h)?" sp-thumb-active":""),h=l.toString(e.preferredFormat||"rgb"),n=k?"background-color:"+l.toRgbString():"filter:"+l.toFilter();f.push('<span title="'+
...
h){function m(){p.showPaletteOnly&&(p.showPalette=!0);Ba.text(p.showPaletteOnly?p.togglePaletteMoreText:p.togglePaletteLessText);if(p.palette){qa=p.palette.slice(0);ga=c.isArray(qa[0])?qa:[qa];ra={};for(var a=0;a<ga.length;a++)for(var b=0;b<ga[a].length;b++){var d=tinycolor(ga[a][b]).toRgbString();ra[d]=!0}}v.toggleClass("sp-flat",M);v.toggleClass("sp-input-disabled",!p.showInput);v.toggleClass("sp-alpha-enabled",p.showAlpha);v.toggleClass("sp-clear-enabled",Y);v.toggleClass("sp-buttons-disabled",
!p.showButtons);v.toggleClass("sp-palette-buttons-disabled",!p.togglePaletteOnly);v.toggleClass("sp-palette-disabled",!p.showPalette);v.toggleClass("sp-palette-only",p.showPaletteOnly);v.toggleClass("sp-initial-disabled",!p.showInitial);v.addClass(p.className).addClass(p.containerClassName);J()}function s(){if(t&&window.localStorage){try{var a=window.localStorage[t].split(",#");1<a.length&&(delete window.localStorage[t],c.each(a,function(a,c){fa(c)}))}catch(b){}try{N=window.localStorage[t].split(";")}catch(d){}}}
function fa(a){if(R){a=tinycolor(a).toRgbString();if(!ra[a]&&-1===c.inArray(a,N))for(N.push(a);N.length>Ua;)N.shift();if(t&&window.localStorage)try{window.localStorage[t]=N.join(";")}catch(b){}}}function Ka(){var a=[];if(p.showPalette)for(var c=0;c<N.length;c++){var b=tinycolor(N[c]).toRgbString();ra[b]||a.push(N[c])}return a.reverse().slice(0,p.maxSelectionSize)}function I(){var a=E(),d=c.map(ga,function(c,d){return b(c,a,"sp-palette-row sp-palette-row-"+d,p)});s();N&&d.push(b(Ka(),a,"sp-palette-row sp-palette-row-selection",
p));La.html(d.join(""))}function w(){if(p.showInitial){var a=Z,c=E();Ma.html(b([a,c],c,"sp-palette-row-initial",p))}}function L(){(0>=K||0>=O||0>=sa)&&J();v.addClass(Na);S=null;B.trigger("dragstart.spectrum",[E()])}function z(){v.removeClass(Na);B.trigger("dragstop.spectrum",[E()])}function W(){var a=T.val();null!==a&&""!==a||!Y?(a=tinycolor(a),a.isValid()?(U(a),H(!0)):T.addClass("sp-validation-error")):(U(null),H(!0))}function ta(){r?aa():oa()}function oa(){var a=c.Event("beforeShow.spectrum");if(r)J();
else if(B.trigger(a,[E()]),!1!==C.beforeShow(E())&&!a.isDefaultPrevented()){for(a=0;a<u.length;a++)u[a]&&u[a].hide();r=!0;c(Ca).bind("click.spectrum",Aa);c(window).bind("resize.spectrum",ba);ca.addClass("sp-active");v.removeClass("sp-hidden");J();Q();Z=E();w();C.show(Z);B.trigger("show.spectrum",[Z])}}function Aa(a){2!=a.button&&(Va?H(!0):U(Z,!0),aa())}function aa(){r&&!M&&(r=!1,c(Ca).unbind("click.spectrum",Aa),c(window).unbind("resize.spectrum",ba),ca.removeClass("sp-active"),v.addClass("sp-hidden"),
C.hide(E()),B.trigger("hide.spectrum",[E()]))}function U(a,c){if(tinycolor.equals(a,E()))Q();else{var b,d;!a&&Y?V=!0:(V=!1,b=tinycolor(a),d=b.toHsv(),ha=d.h%360/360,ia=d.s,ja=d.v,P=d.a);Q();b&&(b.isValid()&&!c)&&(ka=Da||b.getFormat())}}function E(a){a=a||{};return Y&&V?null:tinycolor.fromRatio({h:ha,s:ia,v:ja,a:Math.round(100*P)/100},{format:a.format||ka})}function da(){Q();C.move(E());B.trigger("move.spectrum",[E()])}function Q(){T.removeClass("sp-validation-error");y();var a=tinycolor.fromRatio({h:ha,
s:1,v:1});ua.css("background-color",a.toHexString());a=ka;!(1>P)||0===P&&"name"===a||"hex"!==a&&"hex3"!==a&&"hex6"!==a&&"name"!==a||(a="rgb");var c=E({format:a}),b="";ea.removeClass("sp-clear-display");ea.css("background-color","transparent");if(!c&&Y)ea.addClass("sp-clear-display");else{var b=c.toHexString(),d=c.toRgbString();k||1===c.alpha?ea.css("background-color",d):(ea.css("background-color","transparent"),ea.css("filter",c.toFilter()));if(p.showAlpha){d=c.toRgb();d.a=0;var d=tinycolor(d).toRgbString(),
e="linear-gradient(left, "+d+", "+b+")";n?la.css("filter",tinycolor(d).toFilter({gradientType:1},b)):(la.css("background","-webkit-"+e),la.css("background","-moz-"+e),la.css("background","-ms-"+e),la.css("background","linear-gradient(to right, "+d+", "+b+")"))}b=c.toString(a)}p.showInput&&T.val(b);p.showPalette&&I();w()}function y(){var a=ia,c=ja;Y&&V?(va.hide(),wa.hide(),xa.hide()):(va.show(),wa.show(),xa.show(),a*=O,c=K-c*K,a=Math.max(-D,Math.min(O-D,a-D)),c=Math.max(-D,Math.min(K-D,c-D)),xa.css({top:c+
"px",left:a+"px"}),va.css({left:P*A-Oa/2+"px"}),wa.css({top:ha*sa-Pa+"px"}))}function H(a){var c=E(),b="",d=!tinycolor.equals(c,Z);c&&(b=c.toString(ka),fa(c));ya&&B.val(b);a&&d&&(C.change(c),B.trigger("change",[c]))}function J(){O=ua.width();K=ua.height();D=xa.height();Ea.width();sa=Ea.height();Pa=wa.height();A=Qa.width();Oa=va.width();M||(v.css("position","absolute"),p.offset?v.offset(p.offset):v.offset(f(v,ma)));y();p.showPalette&&I();B.trigger("reflow.spectrum")}function Fa(){aa();Ga=!0;B.attr("disabled",
!0);ma.addClass("sp-disabled")}var p=e(h,d),M=p.flat,R=p.showSelectionPalette,t=p.localStorageKey,F=p.theme,C=p.callbacks,ba=q(J,10),r=!1,O=0,K=0,D=0,sa=0,A=0,Oa=0,Pa=0,ha=0,ia=0,ja=0,P=1,qa=[],ga=[],ra={},N=p.selectionPalette.slice(0),Ua=p.maxSelectionSize,Na="sp-dragging",S=null,Ca=d.ownerDocument,B=c(d),Ga=!1,v=c(X,Ca).addClass(F),Wa=v.find(".sp-picker-container"),ua=v.find(".sp-color"),xa=v.find(".sp-dragger"),Ea=v.find(".sp-hue"),wa=v.find(".sp-slider"),la=v.find(".sp-alpha-inner"),Qa=v.find(".sp-alpha"),
va=v.find(".sp-alpha-handle"),T=v.find(".sp-input"),La=v.find(".sp-palette"),Ma=v.find(".sp-initial"),Ra=v.find(".sp-cancel"),Ha=v.find(".sp-clear"),Sa=v.find(".sp-choose"),Ba=v.find(".sp-palette-toggle"),ya=B.is("input"),Xa=ya&&x&&"color"===B.attr("type"),Ia=ya&&!M,ca=Ia?c(G).addClass(F).addClass(p.className).addClass(p.replacerClassName):c([]),ma=Ia?ca:B,ea=ca.find(".sp-preview-inner"),na=p.color||ya&&B.val(),Z=!1,Da=p.preferredFormat,ka=Da,Va=!p.showButtons||p.clickoutFiresChange,V=!na,Y=p.allowEmpty&&
!Xa;(function(){function a(b){b.data&&b.data.ignore?(U(c(b.target).closest(".sp-thumb-el").data("color")),da()):(U(c(b.target).closest(".sp-thumb-el").data("color")),da(),H(!0),p.hideAfterPaletteSelect&&aa());return!1}n&&v.find("*:not(input)").attr("unselectable","on");m();Ia&&B.after(ca).hide();Y||Ha.hide();if(M)B.after(v).hide();else{var b="parent"===p.appendTo?B.parent():c(p.appendTo);1!==b.length&&(b=c("body"));b.append(v)}s();ma.bind("click.spectrum touchstart.spectrum",function(a){Ga||ta();
a.stopPropagation();c(a.target).is("input")||a.preventDefault()});(B.is(":disabled")||!0===p.disabled)&&Fa();v.click(g);T.change(W);T.bind("paste",function(){setTimeout(W,1)});T.keydown(function(a){13==a.keyCode&&W()});Ra.text(p.cancelText);Ra.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();U(Z,!0);aa()});Ha.attr("title",p.clearText);Ha.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();V=!0;da();M&&H(!0)});Sa.text(p.chooseText);Sa.bind("click.spectrum",
function(a){a.stopPropagation();a.preventDefault();T.hasClass("sp-validation-error")||(H(!0),aa())});Ba.text(p.showPaletteOnly?p.togglePaletteMoreText:p.togglePaletteLessText);Ba.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();p.showPaletteOnly=!p.showPaletteOnly;p.showPaletteOnly||M||v.css("left","-="+(Wa.outerWidth(!0)+5));m()});l(Qa,function(a,c,b){P=a/A;V=!1;b.shiftKey&&(P=Math.round(10*P)/10);da()},L,z);l(Ea,function(a,c){ha=parseFloat(c/sa);V=!1;p.showAlpha||(P=1);da()},
L,z);l(ua,function(a,c,b){b.shiftKey?S||(b=K-ja*K,S=Math.abs(a-ia*O)>Math.abs(c-b)?"x":"y"):S=null;b=!S||"y"===S;S&&"x"!==S||(ia=parseFloat(a/O));b&&(ja=parseFloat((K-c)/K));V=!1;p.showAlpha||(P=1);da()},L,z);na?(U(na),Q(),ka=Da||tinycolor(na).format,fa(na)):Q();M&&oa();b=n?"mousedown.spectrum":"click.spectrum touchstart.spectrum";La.delegate(".sp-thumb-el",b,a);Ma.delegate(".sp-thumb-el:nth-child(1)",b,{ignore:!0},a)})();var za={show:oa,hide:aa,toggle:ta,reflow:J,option:function(b,d){if(b===a)return c.extend({},
p);if(d===a)return p[b];p[b]=d;m()},enable:function(){Ga=!1;B.attr("disabled",!1);ma.removeClass("sp-disabled")},disable:Fa,offset:function(a){p.offset=a;J()},set:function(a){U(a);H()},get:E,destroy:function(){B.show();ma.unbind("click.spectrum touchstart.spectrum");v.remove();ca.remove();u[za.id]=null},container:v};za.id=u.push(za)-1;return za}function f(a,b){var d=a.outerWidth(),e=a.outerHeight(),f=b.outerHeight(),h=a[0].ownerDocument,g=h.documentElement,k=g.clientWidth+c(h).scrollLeft(),h=g.clientHeight+
c(h).scrollTop(),g=b.offset();g.top+=f;g.left-=Math.min(g.left,g.left+d>k&&k>d?Math.abs(g.left+d-k):0);g.top-=Math.min(g.top,g.top+e>h&&h>e?Math.abs(e+f-0):0);return g}function h(){}function g(a){a.stopPropagation()}function m(a,c){var b=Array.prototype.slice,d=b.call(arguments,2);return function(){return a.apply(c,d.concat(b.call(arguments)))}}function l(a,b,d,e){function f(a){a.stopPropagation&&a.stopPropagation();a.preventDefault&&a.preventDefault();a.returnValue=!1}function g(c){if(l){if(n&&9>
...
h;c(a).bind("touchstart mousedown",function(b){(b.which?3==b.which:2==b.button)||l||!1===d.apply(a,arguments)||(l=!0,q=c(a).height(),s=c(a).width(),m=c(a).offset(),c(k).bind(x),c(k.body).addClass("sp-dragging"),u||g(b),f(b))})}function q(a,c,b){var d;return function(){var e=this,f=arguments,g=function(){d=null;a.apply(e,f)};b&&clearTimeout(d);if(b||!d)d=setTimeout(g,c)}}var s={beforeShow:h,move:h,change:h,show:h,hide:h,color:!1,flat:!1,showInput:!1,allowEmpty:!1,showButtons:!0,clickoutFiresChange:!1,
showInitial:!1,showPalette:!1,showPaletteOnly:!1,hideAfterPaletteSelect:!1,togglePaletteOnly:!1,showSelectionPalette:!0,localStorageKey:!1,appendTo:"body",maxSelectionSize:7,cancelText:"cancel",chooseText:"choose",togglePaletteMoreText:"more",togglePaletteLessText:"less",clearText:"Clear Color Selection",noColorSelectedText:"No Color Selected",preferredFormat:!1,className:"",containerClassName:"",replacerClassName:"",showAlpha:!1,theme:"sp-light",palette:["#ffffff #000000 #ff0000 #ff8000 #ffff00 #008000 #0000ff #4b0082 #9400d3".split(" ")],
...
X=function(){var a="";if(n)for(var c=1;6>=c;c++)a+="<div class='sp-"+c+"'></div>";return["<div class='sp-container sp-hidden'><div class='sp-palette-container'><div class='sp-palette sp-thumb sp-cf'></div><div class='sp-palette-button-container sp-cf'><button type='button' class='sp-palette-toggle'></button></div></div><div class='sp-picker-container'><div class='sp-top sp-cf'><div class='sp-fill'></div><div class='sp-top-inner'><div class='sp-color'><div class='sp-sat'><div class='sp-val'><div class='sp-dragger'></div></div></div></div><div class='sp-clear sp-clear-display'></div><div class='sp-hue'><div class='sp-slider'></div>",
...
l;c.fn.spectrum.defaults=s;c.spectrum={};c.spectrum.localization={};c.spectrum.palettes={};c.fn.spectrum.processNativeColorInputs=function(){x||c("input[type=color]").spectrum({preferredFormat:"hex6"})};(function(){function a(c,b,d){c=y(c,255);b=y(b,255);d=y(d,255);var e=C(c,b,d),f=F(c,b,d),g,h=(e+f)/2;if(e==f)g=f=0;else{var k=e-f,f=0.5<h?k/(2-e-f):k/(e+f);switch(e){case c:g=(b-d)/k+(b<d?6:0);break;case b:g=(d-c)/k+2;break;case d:g=(c-b)/k+4}g/=6}return{h:g,s:f,l:h}}function c(a,b,d){function e(a,
c,b){0>b&&(b+=1);1<b&&(b-=1);return b<1/6?a+6*(c-a)*b:0.5>b?c:b<2/3?a+6*(c-a)*(2/3-b):a}a=y(a,360);b=y(b,100);d=y(d,100);if(0===b)d=b=a=d;else{var f=0.5>d?d*(1+b):d+b-d*b,g=2*d-f;d=e(g,f,a+1/3);b=e(g,f,a);a=e(g,f,a-1/3)}return{r:255*d,g:255*b,b:255*a}}function b(a,c,d){a=y(a,255);c=y(c,255);d=y(d,255);var e=C(a,c,d),f=F(a,c,d),g,h=e-f;if(e==f)g=0;else{switch(e){case a:g=(c-d)/h+(c<d?6:0);break;case c:g=(d-a)/h+2;break;case d:g=(a-c)/h+4}g/=6}return{h:g,s:0===e?0:h/e,v:e}}function d(a,c,b,e){a=[H(t(a).toString(16)),
H(t(c).toString(16)),H(t(b).toString(16))];return e&&a[0].charAt(0)==a[0].charAt(1)&&a[1].charAt(0)==a[1].charAt(1)&&a[2].charAt(0)==a[2].charAt(1)?a[0].charAt(0)+a[1].charAt(0)+a[2].charAt(0):a.join("")}function e(a,c,b,d){return[H(Math.round(255*parseFloat(d)).toString(16)),H(t(a).toString(16)),H(t(c).toString(16)),H(t(b).toString(16))].join("")}function f(a,c){c=0===c?0:c||10;var b=r(a).toHsl();b.s-=c/100;b.s=F(1,C(0,b.s));return r(b)}function g(a,c){c=0===c?0:c||10;var b=r(a).toHsl();b.s+=c/100;
b.s=F(1,C(0,b.s));return r(b)}function h(a){return r(a).desaturate(100)}function k(a,c){c=0===c?0:c||10;var b=r(a).toHsl();b.l+=c/100;b.l=F(1,C(0,b.l));return r(b)}function l(a,c){c=0===c?0:c||10;var b=r(a).toRgb();b.r=C(0,F(255,b.r-t(255*-(c/100))));b.g=C(0,F(255,b.g-t(255*-(c/100))));b.b=C(0,F(255,b.b-t(255*-(c/100))));return r(b)}function m(a,c){c=0===c?0:c||10;var b=r(a).toHsl();b.l-=c/100;b.l=F(1,C(0,b.l));return r(b)}function n(a,c){var b=r(a).toHsl(),d=(t(b.h)+c)%360;b.h=0>d?360+d:d;return r(b)}
...
(d.h-(c*b>>1)+720)%360;--b;)d.h=(d.h+c)%360,a.push(r(d));return a}function G(a,b){b=b||6;for(var c=r(a).toHsv(),d=c.h,e=c.s,c=c.v,f=[],g=1/b;b--;)f.push(r({h:d,s:e,v:c})),c=(c+g)%1;return f}function Q(a){a=parseFloat(a);if(isNaN(a)||0>a||1<a)a=1;return a}function y(a,b){"string"==typeof a&&(-1!=a.indexOf(".")&&1===parseFloat(a))&&(a="100%");var c="string"===typeof a&&-1!=a.indexOf("%");a=F(b,C(0,parseFloat(a)));c&&(a=parseInt(a*b,10)/100);return 1E-6>R.abs(a-b)?1:a%b/parseFloat(b)}function H(a){return 1==
a.length?"0"+a:""+a}function J(a){1>=a&&(a=100*a+"%");return a}var X=/^[\s,#]+/,p=/\s+$/,M=0,R=Math,t=R.round,F=R.min,C=R.max,ba=R.random,r=function A(a,b){var d,e,f,g,h;a=a?a:"";b=b||{};if(a instanceof A)return a;if(!(this instanceof A))return new A(a,b);e=a;h={r:0,g:0,b:0};var k=1;f=d=!1;if("string"==typeof e)a:{e=e.replace(X,"").replace(p,"").toLowerCase();var l=!1;if(O[e])e=O[e],l=!0;else if("transparent"==e){e={r:0,g:0,b:0,a:0,format:"name"};break a}e=(g=D.rgb.exec(e))?{r:g[1],g:g[2],b:g[3]}:
...
16),g:parseInt(g[2]+""+g[2],16),b:parseInt(g[3]+""+g[3],16),format:l?"name":"hex"}:!1}if("object"==typeof e){if(e.hasOwnProperty("r")&&e.hasOwnProperty("g")&&e.hasOwnProperty("b"))h={r:255*y(e.r,255),g:255*y(e.g,255),b:255*y(e.b,255)},d=!0,f="%"===String(e.r).substr(-1)?"prgb":"rgb";else if(e.hasOwnProperty("h")&&e.hasOwnProperty("s")&&e.hasOwnProperty("v")){e.s=J(e.s);e.v=J(e.v);f=e.h;l=e.s;h=e.v;f=6*y(f,360);l=y(l,100);h=y(h,100);d=R.floor(f);var m=f-d;f=h*(1-l);g=h*(1-m*l);l=h*(1-(1-m)*l);d%=6;
h={r:255*[h,g,f,f,l,h][d],g:255*[l,h,h,g,f,f][d],b:255*[f,f,l,h,h,g][d]};d=!0;f="hsv"}else e.hasOwnProperty("h")&&(e.hasOwnProperty("s")&&e.hasOwnProperty("l"))&&(e.s=J(e.s),e.l=J(e.l),h=c(e.h,e.s,e.l),d=!0,f="hsl");e.hasOwnProperty("a")&&(k=e.a)}k=Q(k);e=e.format||f;f=F(255,C(h.r,0));g=F(255,C(h.g,0));h=F(255,C(h.b,0));this._originalInput=a;this._r=f;this._g=g;this._b=h;this._a=k;this._roundA=t(100*this._a)/100;this._format=b.format||e;this._gradientType=b.gradientType;1>this._r&&(this._r=t(this._r));
1>this._g&&(this._g=t(this._g));1>this._b&&(this._b=t(this._b));this._ok=d;this._tc_id=M++};r.prototype={isDark:function(){return 128>this.getBrightness()},isLight:function(){return!this.isDark()},isValid:function(){return this._ok},getOriginalInput:function(){return this._originalInput},getFormat:function(){return this._format},getAlpha:function(){return this._a},getBrightness:function(){var a=this.toRgb();return(299*a.r+587*a.g+114*a.b)/1E3},setAlpha:function(a){this._a=Q(a);this._roundA=t(100*
...
f.g)+Math.max(e.b,f.b)-Math.min(e.b,f.b);return{brightness:Math.abs(c-d),color:e}};r.isReadable=function(a,b){var c=r.readability(a,b);return 125<c.brightness&&500<c.color};r.mostReadable=function(a,b){for(var c=null,d=0,e=!1,f=0;f<b.length;f++){var g=r.readability(a,b[f]),h=125<g.brightness&&500<g.color,g=3*(g.brightness/125)+g.color/500;if(h&&!e||h&&e&&g>d||!h&&!e&&g>d)e=h,d=g,c=r(b[f])}return c};var O=r.names={aliceblue:"f0f8ff",antiquewhite:"faebd7",aqua:"0ff",aquamarine:"7fffd4",azure:"f0ffff",
beige:"f5f5dc",bisque:"ffe4c4",black:"000",blanchedalmond:"ffebcd",blue:"00f",blueviolet:"8a2be2",brown:"a52a2a",burlywood:"deb887",burntsienna:"ea7e5d",cadetblue:"5f9ea0",chartreuse:"7fff00",chocolate:"d2691e",coral:"ff7f50",cornflowerblue:"6495ed",cornsilk:"fff8dc",crimson:"dc143c",cyan:"0ff",darkblue:"00008b",darkcyan:"008b8b",darkgoldenrod:"b8860b",darkgray:"a9a9a9",darkgreen:"006400",darkgrey:"a9a9a9",darkkhaki:"bdb76b",darkmagenta:"8b008b",darkolivegreen:"556b2f",darkorange:"ff8c00",darkorchid:"9932cc",
darkred:"8b0000",darksalmon:"e9967a",darkseagreen:"8fbc8f",darkslateblue:"483d8b",darkslategray:"2f4f4f",darkslategrey:"2f4f4f",darkturquoise:"00ced1",darkviolet:"9400d3",deeppink:"ff1493",deepskyblue:"00bfff",dimgray:"696969",dimgrey:"696969",dodgerblue:"1e90ff",firebrick:"b22222",floralwhite:"fffaf0",forestgreen:"228b22",fuchsia:"f0f",gainsboro:"dcdcdc",ghostwhite:"f8f8ff",gold:"ffd700",goldenrod:"daa520",gray:"808080",green:"008000",greenyellow:"adff2f",grey:"808080",honeydew:"f0fff0",hotpink:"ff69b4",
indianred:"cd5c5c",indigo:"4b0082",ivory:"fffff0",khaki:"f0e68c",lavender:"e6e6fa",lavenderblush:"fff0f5",lawngreen:"7cfc00",lemonchiffon:"fffacd",lightblue:"add8e6",lightcoral:"f08080",lightcyan:"e0ffff",lightgoldenrodyellow:"fafad2",lightgray:"d3d3d3",lightgreen:"90ee90",lightgrey:"d3d3d3",lightpink:"ffb6c1",lightsalmon:"ffa07a",lightseagreen:"20b2aa",lightskyblue:"87cefa",lightslategray:"789",lightslategrey:"789",lightsteelblue:"b0c4de",lightyellow:"ffffe0",lime:"0f0",limegreen:"32cd32",linen:"faf0e6",
magenta:"f0f",maroon:"800000",mediumaquamarine:"66cdaa",mediumblue:"0000cd",mediumorchid:"ba55d3",mediumpurple:"9370db",mediumseagreen:"3cb371",mediumslateblue:"7b68ee",mediumspringgreen:"00fa9a",mediumturquoise:"48d1cc",mediumvioletred:"c71585",midnightblue:"191970",mintcream:"f5fffa",mistyrose:"ffe4e1",moccasin:"ffe4b5",navajowhite:"ffdead",navy:"000080",oldlace:"fdf5e6",olive:"808000",olivedrab:"6b8e23",orange:"ffa500",orangered:"ff4500",orchid:"da70d6",palegoldenrod:"eee8aa",palegreen:"98fb98",
paleturquoise:"afeeee",palevioletred:"db7093",papayawhip:"ffefd5",peachpuff:"ffdab9",peru:"cd853f",pink:"ffc0cb",plum:"dda0dd",powderblue:"b0e0e6",purple:"800080",rebeccapurple:"663399",red:"f00",rosybrown:"bc8f8f",royalblue:"4169e1",saddlebrown:"8b4513",salmon:"fa8072",sandybrown:"f4a460",seagreen:"2e8b57",seashell:"fff5ee",sienna:"a0522d",silver:"c0c0c0",skyblue:"87ceeb",slateblue:"6a5acd",slategray:"708090",slategrey:"708090",snow:"fffafa",springgreen:"00ff7f",steelblue:"4682b4",tan:"d2b48c",teal:"008080",
thistle:"d8bfd8",tomato:"ff6347",turquoise:"40e0d0",violet:"ee82ee",wheat:"f5deb3",white:"fff",whitesmoke:"f5f5f5",yellow:"ff0",yellowgreen:"9acd32"},K=r.hexNames=function(a){var b={},c;for(c in a)a.hasOwnProperty(c)&&(b[a[c]]=c);return b}(O),D={rgb:RegExp("rgb[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),rgba:RegExp("rgba[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),
hsl:RegExp("hsl[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),hsla:RegExp("hsla[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),hsv:RegExp("hsv[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),
hsva:RegExp("hsva[\\s|\\(]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))[,|\\s]+((?:[-\\+]?\\d*\\.\\d+%?)|(?:[-\\+]?\\d+%?))\\s*\\)?"),hex3:/^([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/,hex6:/^([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})$/,hex8:/^([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})$/};window.tinycolor=r})();c(function(){c.fn.spectrum.load&&c.fn.spectrum.processNativeColorInputs()})});
