...
DrawTogether.prototype.setRoom=function(b){this.current_room=b;this.roomInput.value=b;location.hash=b+","+this.paint.public.leftTopX+","+this.paint.public.leftTopY+",lol"};DrawTogether.prototype.openSettingsWindow=function(){this.settingsWindow.style.display="block";ga("send","event","openwindow","settings")};
...
DrawTogether.prototype.createDrawZone=function(){var b=this.container.appendChild(document.createElement("div"));b.className="drawtogether-paint-container";this.paintContainer=b;this.paint=new Paint(b);this.userCtx=this.paint.newCanvasOnTop("userinteraction").getContext("2d");this.setLoadImage();this.paint.background.requestUserChunk=function(b,c,e,d){var f=new Image;d=d||0;d+=5E3;f.onload=function(b){e(f)};f.onerror=function(f){console.error("Failed to load chunk ",b,c," retrying in "+Math.round(d/
1E3)+" seconds");setTimeout(function(){this.paint.background.requestUserChunk(b,c,e,d)}.bind(this),d)}.bind(this);var g=encodeURIComponent(this.current_room);b=encodeURIComponent(b);c=encodeURIComponent(c);f.crossOrigin="Anonymous";this.current_room?f.src=this.settings.imageServer+"/chunk?room="+g+"&x="+b+"&y="+c+"&t="+(this.lastImageUpdate||Date.now()):e(f)}.bind(this);this.paint.addEventListener("userdrawing",function(b){if(0!==this.current_room.indexOf("private_")){if(!this.account.uKey&&(10<b.drawing.size&&
...
this.lastPathPoint=b.point}}.bind(this));this.paint.addEventListener("move",function(b){location.hash=this.current_room+","+this.paint.public.leftTopX+","+this.paint.public.leftTopY}.bind(this));this.paint.changeTool("grab")};DrawTogether.prototype.createMessage=function(){this.messageDom=this.container.appendChild(document.createElement("div"));this.messageDom.className="drawtogether-general-message"};
...
c)));return b};
(function(b){"function"===typeof define&&define.amd?define(["jquery"],b):"object"==typeof exports&&"object"==typeof module?module.exports=b:b(jQuery)})(function(b,a){function c(a,c,d,e){for(var f=[],g=0;g<a.length;g++){var h=a[g];if(h){var m=tinycolor(h),k=0.5>m.toHsl().l?"sp-thumb-el sp-thumb-dark":"sp-thumb-el sp-thumb-light",k=k+(tinycolor.equals(c,h)?" sp-thumb-active":""),h=m.toString(e.preferredFormat||"rgb"),l=t?"background-color:"+m.toRgbString():"filter:"+m.toFilter();f.push('<span title="'+h+
'" data-color="'+m.toRgbString()+'" class="'+k+'"><span class="sp-thumb-inner" style="'+l+';" /></span>')}else f.push(b("<div />").append(b('<span data-color="" style="background-color:transparent;" class="sp-clear-display"></span>').attr("title",e.noColorSelectedText)).html())}return"<div class='sp-cf "+d+"'>"+f.join("")+"</div>"}function e(a,c){var d=b.extend({},n,a);d.callbacks={move:k(d.move,c),change:k(d.change,c),show:k(d.show,c),hide:k(d.hide,c),beforeShow:k(d.beforeShow,c)};return d}function d(d,
...
!r.showButtons);x.toggleClass("sp-palette-buttons-disabled",!r.togglePaletteOnly);x.toggleClass("sp-palette-disabled",!r.showPalette);x.toggleClass("sp-palette-only",r.showPaletteOnly);x.toggleClass("sp-initial-disabled",!r.showInitial);x.addClass(r.className).addClass(r.containerClassName);K()}function n(){if(v&&window.localStorage){try{var a=window.localStorage[v].split(",#");1<a.length&&(delete window.localStorage[v],b.each(a,function(b,a){Y(a)}))}catch(c){}try{O=window.localStorage[v].split(";")}catch(d){}}}
function Y(a){if(T){a=tinycolor(a).toRgbString();if(!ua[a]&&-1===b.inArray(a,O))for(O.push(a);O.length>Wa;)O.shift();if(v&&window.localStorage)try{window.localStorage[v]=O.join(";")}catch(c){}}}function Ea(){var b=[];if(r.showPalette)for(var a=0;a<O.length;a++){var c=tinycolor(O[a]).toRgbString();ua[c]||b.push(O[a])}return b.reverse().slice(0,r.maxSelectionSize)}function z(){var a=J(),d=b.map(ka,function(b,d){return c(b,a,"sp-palette-row sp-palette-row-"+d,r)});n();O&&d.push(c(Ea(),a,"sp-palette-row sp-palette-row-selection",
...
!Za;(function(){function a(c){c.data&&c.data.ignore?(W(b(c.target).closest(".sp-thumb-el").data("color")),ia()):(W(b(c.target).closest(".sp-thumb-el").data("color")),ia(),H(!0),r.hideAfterPaletteSelect&&ba());return!1}m&&x.find("*:not(input)").attr("unselectable","on");k();La&&C.after(ha).hide();Z||Ka.hide();if(N)C.after(x).hide();else{var c="parent"===r.appendTo?C.parent():b(r.appendTo);1!==c.length&&(c=b("body"));c.append(x)}n();ra.bind("click.spectrum touchstart.spectrum",function(a){Ja||da();
...
ia()},F,y);l(wa,function(b,a,c){c.shiftKey?U||(c=M-oa*M,U=Math.abs(b-na*P)>Math.abs(a-c)?"x":"y"):U=null;c=!U||"y"===U;U&&"x"!==U||(na=parseFloat(b/P));c&&(oa=parseFloat((M-a)/M));X=!1;r.showAlpha||(Q=1);ia()},F,y);sa?(W(sa),S(),pa=Ha||tinycolor(sa).format,Y(sa)):S();N&&ea();c=m?"mousedown.spectrum":"click.spectrum touchstart.spectrum";Ma.delegate(".sp-thumb-el",c,a);Na.delegate(".sp-thumb-el:nth-child(1)",c,{ignore:!0},a)})();var Ca={show:ea,hide:ba,toggle:da,reflow:K,option:function(c,d){if(c===
...
g=h.clientHeight+b(g).scrollTop(),h=c.offset();h.top+=f;h.left-=Math.min(h.left,h.left+d>t&&t>d?Math.abs(h.left+d-t):0);h.top-=Math.min(h.top,h.top+e>g&&g>e?Math.abs(e+f-0):0);return h}function g(){}function h(b){b.stopPropagation()}function k(b,a){var c=Array.prototype.slice,d=c.call(arguments,2);return function(){return b.apply(a,d.concat(c.call(arguments)))}}function l(a,c,d,e){function f(b){b.stopPropagation&&b.stopPropagation();b.preventDefault&&b.preventDefault();b.returnValue=!1}function g(b){if(k){if(m&&
9>t.documentMode&&!b.button)return h();var d=b.originalEvent&&b.originalEvent.touches,e=d?d[0].pageY:b.pageY,d=Math.max(0,Math.min((d?d[0].pageX:b.pageX)-n.left,p)),e=Math.max(0,Math.min(e-n.top,l));w&&f(b);c.apply(a,[d,e,b])}}function h(){k&&(b(t).unbind(D),b(t.body).removeClass("sp-dragging"),e.apply(a,arguments));k=!1}c=c||function(){};d=d||function(){};e=e||function(){};var t=document,k=!1,n={},l=0,p=0,w="ontouchstart"in window,D={};D.selectstart=f;D.dragstart=f;D["touchmove mousemove"]=g;D["touchend mouseup"]=
h;b(a).bind("touchstart mousedown",function(c){(c.which?3==c.which:2==c.button)||k||!1===d.apply(a,arguments)||(k=!0,l=b(a).height(),p=b(a).width(),n=b(a).offset(),b(t).bind(D),b(t.body).addClass("sp-dragging"),w||g(c),f(c))})}function p(b,a,c){var d;return function(){var e=this,f=arguments,g=function(){d=null;b.apply(e,f)};c&&clearTimeout(d);if(c||!d)d=setTimeout(g,a)}}var n={beforeShow:g,move:g,change:g,show:g,hide:g,color:!1,flat:!1,showInput:!1,allowEmpty:!1,showButtons:!0,clickoutFiresChange:!1,
...
c.s=I(1,E(0,c.s));return u(c)}function h(b){return u(b).desaturate(100)}function t(b,a){a=0===a?0:a||10;var c=u(b).toHsl();c.l+=a/100;c.l=I(1,E(0,c.l));return u(c)}function m(b,a){a=0===a?0:a||10;var c=u(b).toRgb();c.r=E(0,I(255,c.r-v(255*-(a/100))));c.g=E(0,I(255,c.g-v(255*-(a/100))));c.b=E(0,I(255,c.b-v(255*-(a/100))));return u(c)}function k(b,a){a=0===a?0:a||10;var c=u(b).toHsl();c.l-=a/100;c.l=I(1,E(0,c.l));return u(c)}function n(b,a){var c=u(b).toHsl(),d=(v(c.h)+a)%360;c.h=0>d?360+d:d;return u(c)}
function l(b){b=u(b).toHsl();b.h=(b.h+180)%360;return u(b)}function p(b){var a=u(b).toHsl(),c=a.h;return[u(b),u({h:(c+120)%360,s:a.s,l:a.l}),u({h:(c+240)%360,s:a.s,l:a.l})]}function D(b){var a=u(b).toHsl(),c=a.h;return[u(b),u({h:(c+90)%360,s:a.s,l:a.l}),u({h:(c+180)%360,s:a.s,l:a.l}),u({h:(c+270)%360,s:a.s,l:a.l})]}function w(b){var a=u(b).toHsl(),c=a.h;return[u(b),u({h:(c+72)%360,s:a.s,l:a.l}),u({h:(c+216)%360,s:a.s,l:a.l})]}function A(b,a,c){a=a||6;c=c||30;var d=u(b).toHsl();c=360/c;b=[u(b)];for(d.h=
...
b.length?"0"+b:""+b}function K(b){1>=b&&(b=100*b+"%");return b}var Ba=/^[\s,#]+/,r=/\s+$/,N=0,T=Math,v=T.round,I=T.min,E=T.max,fa=T.random,u=function ca(b,c){var d,e,f,g,h;b=b?b:"";c=c||{};if(b instanceof ca)return b;if(!(this instanceof ca))return new ca(b,c);e=b;h={r:0,g:0,b:0};var t=1;f=d=!1;if("string"==typeof e)a:{e=e.replace(Ba,"").replace(r,"").toLowerCase();var m=!1;if(P[e])e=P[e],m=!0;else if("transparent"==e){e={r:0,g:0,b:0,a:0,format:"name"};break a}e=(g=G.rgb.exec(e))?{r:g[1],g:g[2],b:g[3]}:
(g=G.rgba.exec(e))?{r:g[1],g:g[2],b:g[3],a:g[4]}:(g=G.hsl.exec(e))?{h:g[1],s:g[2],l:g[3]}:(g=G.hsla.exec(e))?{h:g[1],s:g[2],l:g[3],a:g[4]}:(g=G.hsv.exec(e))?{h:g[1],s:g[2],v:g[3]}:(g=G.hsva.exec(e))?{h:g[1],s:g[2],v:g[3],a:g[4]}:(g=G.hex8.exec(e))?{a:parseInt(g[1],16)/255,r:parseInt(g[2],16),g:parseInt(g[3],16),b:parseInt(g[4],16),format:m?"name":"hex8"}:(g=G.hex6.exec(e))?{r:parseInt(g[1],16),g:parseInt(g[2],16),b:parseInt(g[3],16),format:m?"name":"hex"}:(g=G.hex3.exec(e))?{r:parseInt(g[1]+""+g[1],
16),g:parseInt(g[2]+""+g[2],16),b:parseInt(g[3]+""+g[3],16),format:m?"name":"hex"}:!1}if("object"==typeof e){if(e.hasOwnProperty("r")&&e.hasOwnProperty("g")&&e.hasOwnProperty("b"))h={r:255*B(e.r,255),g:255*B(e.g,255),b:255*B(e.b,255)},d=!0,f="%"===String(e.r).substr(-1)?"prgb":"rgb";else if(e.hasOwnProperty("h")&&e.hasOwnProperty("s")&&e.hasOwnProperty("v")){e.s=K(e.s);e.v=K(e.v);f=e.h;m=e.s;h=e.v;f=6*B(f,360);m=B(m,100);h=B(h,100);d=T.floor(f);var k=f-d;f=h*(1-m);g=h*(1-k*m);m=h*(1-(1-k)*m);d%=6;
h={r:255*[h,g,f,f,m,h][d],g:255*[m,h,h,g,f,f][d],b:255*[f,f,m,h,h,g][d]};d=!0;f="hsv"}else e.hasOwnProperty("h")&&(e.hasOwnProperty("s")&&e.hasOwnProperty("l"))&&(e.s=K(e.s),e.l=K(e.l),h=a(e.h,e.s,e.l),d=!0,f="hsl");e.hasOwnProperty("a")&&(t=e.a)}t=S(t);e=e.format||f;f=I(255,E(h.r,0));g=I(255,E(h.g,0));h=I(255,E(h.b,0));this._originalInput=b;this._r=f;this._g=g;this._b=h;this._a=t;this._roundA=v(100*this._a)/100;this._format=c.format||e;this._gradientType=c.gradientType;1>this._r&&(this._r=v(this._r));
...
arguments)},complement:function(){return this._applyCombination(l,arguments)},monochromatic:function(){return this._applyCombination(q,arguments)},splitcomplement:function(){return this._applyCombination(w,arguments)},triad:function(){return this._applyCombination(p,arguments)},tetrad:function(){return this._applyCombination(D,arguments)}};u.fromRatio=function(b,a){if("object"==typeof b){var c={},d;for(d in b)b.hasOwnProperty(d)&&(c[d]="a"===d?b[d]:K(b[d]));b=c}return u(b,a)};u.equals=function(b,
...
(function(b){for(var a=CryptoJS,c=a.lib,e=c.WordArray,d=c.Hasher,c=a.algo,f=[],g=[],h=function(b){return 4294967296*(b-(b|0))|0},k=2,l=0;64>l;){var p;a:{p=k;for(var n=b.sqrt(p),q=2;q<=n;q++)if(!(p%q)){p=!1;break a}p=!0}p&&(8>l&&(f[l]=h(b.pow(k,0.5))),g[l]=h(b.pow(k,1/3)),l++);k++}var m=[],c=c.SHA256=d.extend({_doReset:function(){this._hash=new e.init(f.slice(0))},_doProcessBlock:function(b,a){for(var c=this._hash.words,d=c[0],e=c[1],f=c[2],h=c[3],k=c[4],n=c[5],l=c[6],p=c[7],q=0;64>q;q++){if(16>q)m[q]=
b[a+q]|0;else{var F=m[q-15],y=m[q-2];m[q]=((F<<25|F>>>7)^(F<<14|F>>>18)^F>>>3)+m[q-7]+((y<<15|y>>>17)^(y<<13|y>>>19)^y>>>10)+m[q-16]}F=p+((k<<26|k>>>6)^(k<<21|k>>>11)^(k<<7|k>>>25))+(k&n^~k&l)+g[q]+m[q];y=((d<<30|d>>>2)^(d<<19|d>>>13)^(d<<10|d>>>22))+(d&e^d&f^e&f);p=l;l=n;n=k;k=h+F|0;h=f;f=e;e=d;d=F+y|0}c[0]=c[0]+d|0;c[1]=c[1]+e|0;c[2]=c[2]+f|0;c[3]=c[3]+h|0;c[4]=c[4]+k|0;c[5]=c[5]+n|0;c[6]=c[6]+l|0;c[7]=c[7]+p|0},_doFinalize:function(){var a=this._data,c=a.words,d=8*this._nDataBytes,e=8*a.sigBytes;
...
this._controls[b]={container:h,control:k,label:l,callback:g};c="input";this._isIE()&&(c="change");var p=this._globalChangeHandler,n=this;k.addEventListener(c,function(){l.innerHTML="<b>"+b+":</b> "+k.value;g&&g(parseFloat(k.value));p&&p();n._callCallbacks(b,{value:parseFloat(k.value)})})},_isIE:function(){return-1!=navigator.userAgent.indexOf("rv:11")||-1!=navigator.userAgent.indexOf("MSIE")?!0:!1},getRangeValue:function(b){return this._controls[b].control.value},setRangeValue:function(b,c,e){var d=
...
b),f.startDragY=h.page("Y",b),f.dragWatchers={current:0,last:0,hold:0,state:""},f.simpleStates={opening:null,towards:null,hyperExtending:null,halfway:null,flick:null,translation:{absolute:0,relative:0,sinceDirectionChange:0,percentage:0}}},dragging:function(b){if(f.isDragging&&d.touchToDrag){var c=h.page("X",b),e=h.page("Y",b),g=f.translation,m=k.translate.get.matrix(4),t=c-f.startDragX,w=0<m,A=t;if(!f.intentChecked||f.hasIntent){d.addBodyClasses&&(0<m?(h.klass.add(a.body,"snapjs-left"),h.klass.remove(a.body,
"snapjs-right")):0>m&&(h.klass.add(a.body,"snapjs-right"),h.klass.remove(a.body,"snapjs-left")));if(!1===f.hasIntent||null===f.hasIntent){var e=h.angleOfDrag(c,e),D=0<=e&&e<=d.slideIntent||360>=e&&e>360-d.slideIntent;f.hasIntent=180<=e&&e<=180+d.slideIntent||180>=e&&e>=180-d.slideIntent||D?!0:!1;f.intentChecked=!0}d.minDragDistance>=Math.abs(c-f.startDragX)||!1===f.hasIntent||(h.events.prevent(b),h.dispatchEvent("drag"),f.dragWatchers.current=c,f.dragWatchers.last>c?("left"!==f.dragWatchers.state&&
(f.dragWatchers.state="left",f.dragWatchers.hold=c),f.dragWatchers.last=c):f.dragWatchers.last<c&&("right"!==f.dragWatchers.state&&(f.dragWatchers.state="right",f.dragWatchers.hold=c),f.dragWatchers.last=c),w?(d.maxPosition<m&&(b=(m-d.maxPosition)*d.resistance,A=t-b),f.simpleStates={opening:"left",towards:f.dragWatchers.state,hyperExtending:d.maxPosition<m,halfway:m>d.maxPosition/2,flick:Math.abs(f.dragWatchers.current-f.dragWatchers.hold)>d.flickThreshold,translation:{absolute:m,relative:t,sinceDirectionChange:f.dragWatchers.current-
f.dragWatchers.hold,percentage:100*(m/d.maxPosition)}}):(d.minPosition>m&&(b=(m-d.minPosition)*d.resistance,A=t-b),f.simpleStates={opening:"right",towards:f.dragWatchers.state,hyperExtending:d.minPosition>m,halfway:m<d.minPosition/2,flick:Math.abs(f.dragWatchers.current-f.dragWatchers.hold)>d.flickThreshold,translation:{absolute:m,relative:t,sinceDirectionChange:f.dragWatchers.current-f.dragWatchers.hold,percentage:100*(m/d.minPosition)}}),k.translate.x(A+g))}}},endDrag:function(b){if(f.isDragging){h.dispatchEvent("end");
...
a),n.send([{type:"ping",data:"probe"}]),n.once("packet",function(c){q||("pong"==c.type&&"probe"==c.data?(k('probe transport "%s" pong',a),z.upgrading=!0,z.emit("upgrading",n),e.priorWebsocketSuccess="websocket"==n.name,k('pausing current transport "%s"',z.transport.name),z.transport.pause(function(){q||("closed"==z.readyState||"closing"==z.readyState)||(k("changing transport and sending upgrade packet"),m(),z.setTransport(n),n.send([{type:"upgrade"}]),z.emit("upgrade",n),n=null,z.upgrading=!1,z.flush())})):
(k('probe transport "%s" failed',a),c=Error("probe error"),c.transport=n.name,z.emit("upgradeError",c)))}))}function d(){q||(q=!0,m(),n.close(),n=null)}function g(c){var e=Error("probe error: "+c);e.transport=n.name;d();k('probe transport "%s" failed because of error: %s',a,c);z.emit("upgradeError",e)}function h(){g("transport closed")}function p(){g("socket closed")}function l(a){n&&a.name!=n.name&&(k('"%s" works - aborting "%s"',a.name,n.name),d())}function m(){n.removeListener("open",c);n.removeListener("error",
g);n.removeListener("close",h);z.removeListener("close",p);z.removeListener("upgrading",l)}k('probing transport "%s"',a);var n=this.createTransport(a,{probe:1}),q=!1,z=this;e.priorWebsocketSuccess=!1;n.once("open",c);n.once("error",g);n.once("close",h);this.once("close",p);this.once("upgrading",l);n.open()};e.prototype.onOpen=function(){k("socket open");this.readyState="open";e.priorWebsocketSuccess="websocket"==this.transport.name;this.emit("open");this.flush();if("open"==this.readyState&&this.upgrade&&
...
p=a("./polling"),n=a("component-emitter"),q=a("component-inherit"),m=a("debug")("engine.io-client:polling-xhr");c.exports=g;c.exports.Request=h;q(g,p);g.prototype.supportsBinary=!0;g.prototype.request=function(a){a=a||{};a.uri=this.uri();a.xd=this.xd;a.xs=this.xs;a.agent=this.agent||!1;a.supportsBinary=this.supportsBinary;a.enablesXDR=this.enablesXDR;return new h(a)};g.prototype.doWrite=function(a,c){var d=this.request({method:"POST",data:a,isBinary:"string"!==typeof a&&void 0!==a}),e=this;d.on("success",
c);d.on("error",function(a){e.onError("xhr post error",a)});this.sendXhr=d};g.prototype.doPoll=function(){m("xhr poll");var a=this.request(),c=this;a.on("data",function(a){c.onData(a)});a.on("error",function(a){c.onError("xhr poll error",a)});this.pollXhr=a};n(h.prototype);h.prototype.create=function(){var a=this.xhr=new l({agent:this.agent,xdomain:this.xd,xscheme:this.xs,enablesXDR:this.enablesXDR}),c=this;try{m("xhr open %s: %s",this.method,this.uri);a.open(this.method,this.uri,this.async);this.supportsBinary&&
...
return k};e.decode=function(c){var e=0.75*c.length,h=c.length,k=0,l,p,n,q;"="===c[c.length-1]&&(e--,"="===c[c.length-2]&&e--);for(var m=new ArrayBuffer(e),t=new Uint8Array(m),e=0;e<h;e+=4)l=a.indexOf(c[e]),p=a.indexOf(c[e+1]),n=a.indexOf(c[e+2]),q=a.indexOf(c[e+3]),t[k++]=l<<2|p>>4,t[k++]=(p&15)<<4|n>>2,t[k++]=(n&3)<<6|q&63;return m}})("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/")},{}],26:[function(a,c,e){(function(a){function e(a,c){c=c||{};for(var d=new g,f=0;f<a.length;f++)d.append(a[f]);
...
56320==(g&64512)?c.push(((f&1023)<<10)+(g&1023)+65536):(c.push(f),d--)):c.push(f);return c}function h(){if(w>=t)throw Error("Invalid byte index");var a=m[w]&255;w++;if(128==(a&192))return a&63;throw Error("Invalid continuation byte");}function k(){var a,c,d,e;if(w>t)throw Error("Invalid byte index");if(w==t)return!1;a=m[w]&255;w++;if(0==(a&128))return a;if(192==(a&224)){c=h();a=(a&31)<<6|c;if(128<=a)return a;throw Error("Invalid continuation byte");}if(224==(a&240)){c=h();d=h();a=(a&15)<<12|c<<6|
d;if(2048<=a)return a;throw Error("Invalid continuation byte");}if(240==(a&248)&&(c=h(),d=h(),e=h(),a=(a&15)<<18|c<<12|d<<6|e,65536<=a&&1114111>=a))return a;throw Error("Invalid UTF-8 detected");}var l="object"==typeof e&&e,p="object"==typeof c&&c&&c.exports==l&&c,n="object"==typeof a&&a;if(n.global===n||n.window===n)f=n;var q=String.fromCharCode,m,t,w,n={version:"2.0.0",encode:function(a){a=g(a);for(var c=a.length,d=-1,e,f="";++d<c;){e=a[d];if(0==(e&4294967168))e=q(e);else{var h="";0==(e&4294965248)?
h=q(e>>6&31|192):0==(e&4294901760)?(h=q(e>>12&15|224),h+=q(e>>6&63|128)):0==(e&4292870144)&&(h=q(e>>18&7|240),h+=q(e>>12&63|128),h+=q(e>>6&63|128));e=h+=q(e&63|128)}f+=e}return f},decode:function(a){m=g(a);t=m.length;w=0;a=[];for(var c;!1!==(c=k());)a.push(c);c=a.length;for(var d=-1,e,f="";++d<c;)e=a[d],65535<e&&(e-=65536,f+=q(e>>>10&1023|55296),e=56320|e&1023),f+=q(e);return f}};if(l&&!l.nodeType)if(p)p.exports=n;else{f={}.hasOwnProperty;for(var A in n)f.call(n,A)&&(l[A]=n[A])}else f.utf8=n})(this)}).call(this,
...
f="source protocol authority userInfo user password host port relative path directory file query anchor".split(" ");c.exports=function(a){var c=a,e=a.indexOf("["),l=a.indexOf("]");-1!=e&&-1!=l&&(a=a.substring(0,e)+a.substring(e,l).replace(/:/g,";")+a.substring(l,a.length));a=d.exec(a||"");for(var p={},n=14;n--;)p[f[n]]=a[n]||"";-1!=e&&-1!=l&&(p.source=c,p.host=p.host.substring(1,p.host.length-1).replace(/;/g,":"),p.authority=p.authority.replace("[","").replace("]","").replace(/;/g,":"),p.ipv6uri=
...
a._placeholder)return c[a.num];if(f(a))for(var e=0;e<a.length;e++)a[e]=d(a[e]);else if(a&&"object"==typeof a)for(e in a)a[e]=d(a[e]);return a}a.data=d(a.data);a.attachments=void 0;return a};e.removeBlobs=function(a,e){function l(a,h,t){if(!a)return a;if(c.Blob&&a instanceof Blob||c.File&&a instanceof File){p++;var w=new FileReader;w.onload=function(){t?t[h]=this.result:n=this.result;--p||e(n)};w.readAsArrayBuffer(a)}else if(f(a))for(w=0;w<a.length;w++)l(a[w],w,a);else if(a&&"object"==typeof a&&!g(a))for(w in a)l(a[w],
w,a)}var p=0,n=a;l(n);p||e(n)}}).call(this,"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{"./is-buffer":41,isarray:42}],40:[function(a,c,e){function d(){}function f(a){var c,d=!1;c=""+a.type;if(e.BINARY_EVENT==a.type||e.BINARY_ACK==a.type)c+=a.attachments,c+="-";a.nsp&&"/"!=a.nsp&&(d=!0,c+=a.nsp);null!=a.id&&(d&&(c+=",",d=!1),c+=a.id);null!=a.data&&(d&&(c+=","),c+=p.stringify(a.data));l("encoded %j as %s",a,c);return c}function g(a,c){n.removeBlobs(a,function(a){var d=n.deconstructPacket(a);
a=f(d.packet);d=d.buffers;d.unshift(a);c(d)})}function h(){this.reconstructor=null}function k(a){this.reconPack=a;this.buffers=[]}var l=a("debug")("socket.io-parser"),p=a("json3");a("isarray");c=a("component-emitter");var n=a("./binary"),q=a("./is-buffer");e.protocol=4;e.types="CONNECT DISCONNECT EVENT BINARY_EVENT ACK BINARY_ACK ERROR".split(" ");e.CONNECT=0;e.DISCONNECT=1;e.EVENT=2;e.ACK=3;e.ERROR=4;e.BINARY_EVENT=5;e.BINARY_ACK=6;e.Encoder=d;e.Decoder=h;d.prototype.encode=function(a,c){l("encoding packet %j",
...
var d;if("bug-string-char-index"==a)d="a"!="a"[0];else if("json"==a)d=c("json-stringify")&&c("json-parse");else{var e;if("json-stringify"==a){d=n.stringify;var h="function"==typeof d&&q;if(h){(e=function(){return 1}).toJSON=e;try{h="0"===d(0)&&"0"===d(new Number)&&'""'==d(new String)&&d(g)===l&&d(l)===l&&d()===l&&"1"===d(e)&&"[1]"==d([e])&&"[null]"==d([l])&&"null"==d(null)&&"[null,null,null]"==d([l,g,null])&&'{"a":[1,true,false,null,"\\u0000\\b\\n\\f\\r\\t"]}'==d({a:[e,!0,!1,null,"\x00\b\n\f\r\t"]})&&
...
d("01")}catch(m){}if(p)try{p=1!==d("1.")}catch(s){}}}}catch(r){p=!1}d=p}}return c[a]=!!d}var g={}.toString,h,k,l,p="object"==typeof JSON&&JSON,n="object"==typeof e&&e&&!e.nodeType&&e;n&&p?(n.stringify=p.stringify,n.parse=p.parse):n=a.JSON=p||{};var q=new Date(-0xc782b5b800cec);try{q=-109252==q.getUTCFullYear()&&0===q.getUTCMonth()&&1===q.getUTCDate()&&10==q.getUTCHours()&&37==q.getUTCMinutes()&&6==q.getUTCSeconds()&&708==q.getUTCMilliseconds()}catch(m){}if(!c("json")){var t=c("bug-string-char-index");
...
for(f&&(h=a.split(""));d<e;d++){var g=a.charCodeAt(d);switch(g){case 8:case 9:case 10:case 12:case 13:case 34:case 92:c+=Va[g];break;default:if(32>g){c+="\\u00"+R(2,g.toString(16));break}c+=f?h[d]:t?a.charAt(d):a[d]}}return c+'"'},Y=function(a,c,d,e,f,n,p){var m,q,r,s,t,v,y,A,z;try{m=c[a]}catch(u){}if("object"==typeof m&&m)if(q=g.call(m),"[object Date]"!=q||h.call(m,"toJSON"))"function"==typeof m.toJSON&&("[object Number]"!=q&&"[object String]"!=q&&"[object Array]"!=q||h.call(m,"toJSON"))&&(m=m.toJSON(a));
else if(m>-1/0&&m<1/0){if(D){s=w(m/864E5);for(q=w(s/365.2425)+1970-1;D(q+1,0)<=s;q++);for(r=w((s-D(q,0))/30.42);D(q,r+1)<=s;r++);s=1+s-D(q,r);t=(m%864E5+864E5)%864E5;v=w(t/36E5)%24;y=w(t/6E4)%60;A=w(t/1E3)%60;t%=1E3}else q=m.getUTCFullYear(),r=m.getUTCMonth(),s=m.getUTCDate(),v=m.getUTCHours(),y=m.getUTCMinutes(),A=m.getUTCSeconds(),t=m.getUTCMilliseconds();m=(0>=q||1E4<=q?(0>q?"-":"+")+R(6,0>q?-q:q):R(4,q))+"-"+R(2,r+1)+"-"+R(2,s)+"T"+R(2,v)+":"+R(2,y)+":"+R(2,A)+"."+R(3,t)+"Z"}else m=null;d&&(m=
d.call(c,a,m));if(null===m)return"null";q=g.call(m);if("[object Boolean]"==q)return""+m;if("[object Number]"==q)return m>-1/0&&m<1/0?""+m:"null";if("[object String]"==q)return Da(""+m);if("object"==typeof m){for(a=p.length;a--;)if(p[a]===m)throw TypeError();p.push(m);z=[];c=n;n+=f;if("[object Array]"==q){r=0;for(a=m.length;r<a;r++)q=Y(r,m,d,e,f,n,p),z.push(q===l?"null":q);a=z.length?f?"[\n"+n+z.join(",\n"+n)+"\n"+c+"]":"["+z.join(",")+"]":"[]"}else k(e||m,function(a){var c=Y(a,m,d,e,f,n,p);c!==l&&
z.push(Da(a)+":"+(f?" ":"")+c)}),a=z.length?f?"{\n"+n+z.join(",\n"+n)+"\n"+c+"}":"{"+z.join(",")+"}":"{}";p.pop();return a}};n.stringify=function(a,c,d){var e,f,h,k;if("function"==typeof c||"object"==typeof c&&c)if("[object Function]"==(k=g.call(c)))f=c;else if("[object Array]"==k){h={};for(var l=0,n=c.length,m;l<n;m=c[l++],(k=g.call(m),"[object String]"==k||"[object Number]"==k)&&(h[m]=1));}if(d)if("[object Number]"==(k=g.call(d))){if(0<(d-=d%1))for(e="",10<d&&(d=10);e.length<d;e+=" ");}else"[object String]"==
...
g.call(e))for(f=e.length;f--;)ea(e,f,d);else k(e,function(a){ea(e,a,d)});return d.call(a,c,e)};n.parse=function(a,c){var d,e;s=0;F=""+a;d=da(L());"$"!=L()&&y();s=F=null;return c&&"[object Function]"==g.call(c)?la((e={},e[""]=d,e),"",c):d}}}})(this)},{}],44:[function(a,c,e){c.exports=function(a,c){for(var e=[],h=(c=c||0)||0;h<a.length;h++)e[h-c]=a[h];return e}},{}]},{},[1])(1)});
